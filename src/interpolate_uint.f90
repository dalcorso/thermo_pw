...
SUBROUTINE interpolate_uint_ptt(celldm_p, uint_geo_eos_t, uint_ptt, tau_ptt, &
                                ...
                                tau_ptt_m1, celldm_p1, celldm_m1, itemp)
!-----------------------------------------------------------------------
!
!  This subroutine receives the internal parameters that at each
!  set of external parameters minimize the free energy and interpolates
...
REAL(DP), INTENT(IN)  :: uint_geo_eos_t(nint_var, ntemp, tot_ngeo_eos), &
                         celldm_p(6,npress), celldm_p1(6,npress),       &
                         celldm_m1(6,npress)
...
INTEGER :: ipress, igeo, nvar, ndata, startp, lastp, ivar
INTEGER :: compute_nwork_ph
REAL(DP), ALLOCATABLE :: x(:,:), x_pos_min(:), x_pos_min_p1(:), &
                         x_pos_min_m1(:), f(:), f_p1(:), f_m1(:)
TYPE(poly1) :: p1, p1_p1, p1_m1
TYPE(poly2) :: p2, p2_p1, p2_m1
TYPE(poly3) :: p3, p3_p1, p3_m1
TYPE(poly4) :: p4, p4_p1, p4_m1

IF (.NOT.linternal_thermo.AND..NOT.linterpolate_tau) RETURN
nvar=crystal_parameters(ibrav)
!
ndata = compute_nwork_ph(no_ph_eos,tot_ngeo_eos)
...
!
      CALL compress_celldm(celldm_p(1,ipress), x_pos_min, nvar, ibrav)
      CALL compress_celldm(celldm_p1(1,ipress), x_pos_min_p1, nvar, ibrav)
      CALL compress_celldm(celldm_m1(1,ipress), x_pos_min_m1, nvar, ibrav)
...
!
         CALL evaluate_fit_quartic(nvar, x_pos_min, uint_ptt(ivar,ipress), p4)
         CALL evaluate_fit_quartic(nvar, x_pos_min_p1, &
                                uint_ptt_p1(ivar,ipress), p4_p1)
         CALL evaluate_fit_quartic(nvar, x_pos_min_m1, &
                                uint_ptt_m1(ivar,ipress), p4_m1)
         CALL clean_poly(p4)
...
!
         CALL evaluate_fit_cubic(nvar, x_pos_min, uint_ptt(ivar,ipress), p3)
         CALL evaluate_fit_cubic(nvar, x_pos_min_p1, &
                             uint_ptt_p1(ivar,ipress),  p3_p1)
         CALL evaluate_fit_cubic(nvar, x_pos_min_m1, &
                             uint_ptt_m1(ivar,ipress), p3_m1)
         CALL clean_poly(p3)
...
!
         CALL evaluate_fit_quadratic(nvar, x_pos_min,uint_ptt(ivar,ipress), &
                                                                          p2)
         CALL evaluate_fit_quadratic(nvar, x_pos_min_p1,&
                             uint_ptt_p1(ivar,ipress), p2_p1)
         CALL evaluate_fit_quadratic(nvar, x_pos_min_m1,&
                             uint_ptt_m1(ivar,ipress), p2_m1)
         CALL clean_poly(p2)
...
!
         CALL evaluate_fit_linear(nvar, x_pos_min, uint_ptt(ivar,ipress), p1)
         CALL evaluate_fit_linear(nvar, x_pos_min_p1, &
                                  uint_ptt_p1(ivar,ipress), p1_p1)
         CALL evaluate_fit_linear(nvar, x_pos_min_m1, &
                                  uint_ptt_m1(ivar,ipress), p1_m1)
         CALL clean_poly(p1)
...
!
!-----------------------------------------------------------------------
SUBROUTINE interpolate_uint_z1ptt(celldm_p, uint_geo_eos, uint_ptt, tau_ptt, &
                                uint_ptt_p1, tau_ptt_p1, uint_ptt_m1,    &
                                tau_ptt_m1, celldm_p1, celldm_m1)
!-----------------------------------------------------------------------
!
!  This subroutine receives the internal parameters that at each
!  set of external parameters and interpolates them at the celldm_p,
!  celldm_p1 and celldm_m1.
!
USE kinds,          ONLY : DP
USE cell_base,      ONLY : ibrav
USE ions_base,      ONLY : nat
...
INTEGER :: ipress, igeo, nvar, ndata, startp, lastp, ivar
INTEGER :: compute_nwork_ph
REAL(DP), ALLOCATABLE :: x(:,:), x_pos_min(:), x_pos_min_m1(:), &
                         x_pos_min_p1(:), f(:)
TYPE(poly1) :: p1
TYPE(poly2) :: p2
TYPE(poly3) :: p3
...
!
         CALL evaluate_fit_quartic(nvar, x_pos_min, uint_ptt(ivar,ipress), p4)
         CALL evaluate_fit_quartic(nvar, x_pos_min_p1, &
                                       uint_ptt_p1(ivar,ipress), p4)
         CALL evaluate_fit_quartic(nvar, x_pos_min_m1, &
                                       uint_ptt_m1(ivar,ipress), p4)
         CALL clean_poly(p4)
      ELSEIF (poly_degree_thermo == 3) THEN
!
!   compute the coefficients of the cubic polynomial
...
!
         CALL evaluate_fit_cubic(nvar, x_pos_min, uint_ptt(ivar,ipress), p3)
         CALL evaluate_fit_cubic(nvar, x_pos_min_p1, &
                            uint_ptt_p1(ivar,ipress), p3)
         CALL evaluate_fit_cubic(nvar, x_pos_min_m1, &
                            uint_ptt_m1(ivar,ipress), p3)
         CALL clean_poly(p3)
      ELSEIF (poly_degree_thermo == 2) THEN
!
!   compute the coefficients of the quadratic polynomial
...
!
         CALL evaluate_fit_quadratic(nvar, x_pos_min,uint_ptt(ivar,ipress), &
                                                                          p2)
         CALL evaluate_fit_quadratic(nvar, x_pos_min_p1,&
                                     uint_ptt_p1(ivar,ipress), p2)
         CALL evaluate_fit_quadratic(nvar, x_pos_min_m1,&
                                     uint_ptt_m1(ivar,ipress), p2)
         CALL clean_poly(p2)
      ELSEIF (poly_degree_thermo == 1) THEN
!
!   compute the coefficients of the linear polynomial
...
!
         CALL evaluate_fit_linear(nvar, x_pos_min, uint_ptt(ivar,ipress), p1)
         CALL evaluate_fit_linear(nvar, x_pos_min_p1, &
                                        uint_ptt_p1(ivar,ipress), p1)
         CALL evaluate_fit_linear(nvar, x_pos_min_m1, &
                                        uint_ptt_m1(ivar,ipress), p1)
         CALL clean_poly(p1)
      ENDIF
   ENDDO
ENDDO
!
!  collect all pressures on all processors
!
CALL mp_sum(uint_ptt, world_comm)
CALL mp_sum(uint_ptt_p1, world_comm)
CALL mp_sum(uint_ptt_m1, world_comm)
...
DO ipress=1, npress
   CALL internal_to_tau(celldm_p(1,ipress), tau_ptt(1,1,ipress),             &
                 uint_ptt(1,ipress), nat, nint_var, iconstr_internal, 1)
   CALL internal_to_tau(celldm_p1(1,ipress), tau_ptt_p1(1,1,ipress),          &
                 uint_ptt_p1(1,ipress), nat, nint_var, iconstr_internal, 1)
   CALL internal_to_tau(celldm_m1(1,ipress), tau_ptt_m1(1,1,ipress),          &
                 uint_ptt_m1(1,ipress), nat, nint_var, iconstr_internal, 1)
ENDDO
...
!
RETURN
END SUBROUTINE interpolate_uint_z1ptt
!
