...
!
!----------------------------------------------------------------------
SUBROUTINE write_mur_pol(omega0, b0in, b01, emin, a_t, m1, itemp)
!----------------------------------------------------------------------
!
!  This routine writes on file the Murnaghan energy versus volume
!  curve, together with the pressure versus volume curve. 
...
REAL(DP), INTENT(IN) :: emin, omega0, b0in, b01, a_t(m1)
INTEGER, INTENT(IN) :: itemp
CHARACTER(LEN=256)   :: filename
CHARACTER(LEN=6) :: int_to_char
REAL(DP) :: omega, e, p, b0
...
IF (ionode) THEN
   ...
   DO i=1,nvol
      omega= vmin_input + deltav * (i-1)
      e=emin+compute_mur_fun(omega, omega0, b0in/ry_kbar, b01, a_t, m1)
      p = b0in * ((omega0 / omega)**b01 - 1.0_DP) / b01 
      DO j=2,m1
         p = p - a_t(j) * (j-1.0_DP) * omega**(j-2) * ry_kbar
      ENDDO
      WRITE(iu_mur,'(f18.10,3f20.10)') omega, e, e+p*omega/ry_kbar, &
                                                 p + pressure_kb
   ENDDO 
