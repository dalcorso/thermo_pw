...
!
!------------------------------------------------------------------------
SUBROUTINE initialize_piezom_tensor( ngeom, nwork )
!------------------------------------------------------------------------
USE kinds,               ONLY : DP
...
USE control_piezomagnetic_tensor, ONLY : piezom_tensor_geo,               &
                                         piezo_zeum_geo
USE control_piezoelectric_tensor, ONLY : dtau_dint_pt, dint_depsilon_geo, &
                                         decompose_piezo
USE elastic_constants,   ONLY : epsilon_voigt, epsilon_geo, sigma_geo
USE piezoelectric_tensor, ONLY : set_piezo_strain_list
USE piezomagnetic_tensor, ONLY : allocate_piezom
USE thermo_sym,          ONLY : b_birss_code
...
IF (b_birss_code==0) RETURN

CALL set_piezo_strain_list(b_birss_code, ibrav, strain_list, nstep)
!
...
IF (decompose_piezo) THEN
   ...
   ALLOCATE(piezo_zeum_geo(3,3,nat,ngeom))
...
RETURN
END SUBROUTINE initialize_piezom_tensor
