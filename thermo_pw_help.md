...
The <code>git</code> version can be used only together with the version of <code>QE</code> reported here: <code>{\color{red}6.2.1}</code>. Please note that sometimes the <code>git</code> version is not working properly and in any case its use is not recommended.</p> 
