...
!
  ...
  ENDIF
!
  IF (nimage > 1 .AND. use_ph_images ) CALL image_q_irr_tpw(distribute_irrep)
!
  IF (recover) THEN
!
...
!-------------------------------------------------------------------------
  SUBROUTINE image_q_irr_tpw(distribute_irrep)
!-------------------------------------------------------------------------
!
...
!-------------------------------------------------------------------------
   ...
   IF (.NOT.ALLOCATED(irr_iq)) RETURN
   DO iq = 1, nqs
      IF (comp_iq(iq)) THEN
   ...
   RETURN 
   END SUBROUTINE print_image_work
