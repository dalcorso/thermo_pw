...
! May     2026 Andrea Dal Corso added k pools parallelization
! June    2026 Andrea Dal Corso added the possibility to use symmetry
!              groups smaller than the point group of the Bravais
!              lattice to reduce the k points in the perpendicular direction.
!
!##############################################################################!
!#                                                                            #!
!#                                                                            #!
...
!  --- Internal definitions ---
   ...
   INTEGER, ALLOCATABLE :: l_cal_int(:) ! flag for occupied/empty states
   REAL(DP) :: t1,t !!REC
   REAL(DP) :: dk(3)
   REAL(DP) :: dk2
   REAL(DP) :: dkmod
...
!  -------------------------------------------------------------------------   !
   ...
   CALL initialize_bp_pool(k_pool_base, nppstr, nkstot) 
...
!  --- Check that k-points form strings ---
   DO i=1,nspin_lsda*nkort
      DO j=2,nppstr
         kindex1=j+(i-1)*nppstr
         IF (.NOT.is_in_my_pool(kindex1, k_pool_base, nks)) CYCLE
         kindex=kindex1-k_pool_base
...
!       or 1+1 (spin) ---
   DO is=1,nspin_lsda
      ...
      DO kort=1,nkort
         ...
         IF (.NOT.is_in_my_pool(nppstr*istring, k_pool_base, nks)) CYCLE
         wstring(istring)=wk(nppstr*istring-k_pool_base)
         weight=weight+wstring(istring)
      END DO
      CALL mp_sum(weight, inter_pool_comm)
      DO kort=1,nkort
         istring=kort+(is-1)*nkort
         IF (.NOT.is_in_my_pool(nppstr*istring, k_pool_base, nks)) CYCLE
      ...
      END DO
   END DO
   CALL mp_sum(wstring, inter_pool_comm)
!  -------------------------------------------------------------------------   !
...
!  --- Start loop over spin ---
   DO is=1,nspin_lsda
      ...
      l_cal_int=0
      DO nb = 1, nbnd
         IF (.NOT.is_in_my_pool(1+nkstot*(is-1)/2, k_pool_base, nks)) CYCLE
         IF (wg(nb,1+nkstot*(is-1)/2-k_pool_base) > eps) l_cal_int(nb)=1
      END DO
      CALL mp_sum(l_cal_int, inter_pool_comm)

      l_cal(1:nbnd)=(l_cal_int(1:nbnd)>0)
...
!        --- Index for this string ---
         istring=kort+(is-1)*nkort
         kpoint1= istring * nppstr
         IF (.NOT.is_in_my_pool(kpoint1, k_pool_base, nks)) CYCLE
...
!  --- End loop over spin ---
   END DO
   DEALLOCATE ( l_cal ) 
   DEALLOCATE ( l_cal_int ) 
!
!  collect the strings phases among all pools
!
   CALL mp_sum(phik, inter_pool_comm)
   CALL mp_sum(cphik, inter_pool_comm)
...
!           & /)")
   ...
   IF(nspin_lsda.gt.1)THEN
      ...
      DO istring=1,nstring/nspin_lsda
         ...
         WRITE( stdout,"(3X,' up ',3X,I5,F14.6,4X,3(F8.4),F17.9)") &
      ...
      DO istring=nstring/2+1,nstring
         ind1=1+(istring-1)*nppstr
         WRITE( stdout,"(3X,'down',3X,I4,F15.6,4X,3(F8.4),F17.9)") &
              &    istring,wstring(istring), xk(1,ind1),xk(2,ind1),xk(3,ind1), &
              &    pdl_elec(istring)
      END DO
      WRITE( stdout,"(2X,76('-'))")
      WRITE( stdout,"(40X,'Average phase (up): ',F17.9)") & 
        pdl_elec_up
      WRITE( stdout,"(38X,'Average phase (down): ',F17.9)")& 
        pdl_elec_dw
   ENDIF
!  --- Treat unpolarized/polarized spin cases ---
   IF (nspin_lsda == 1) THEN
      ...
      DO istring=1,nstring/nspin_lsda
         ind1=1+(istring-1)*nppstr
         WRITE( stdout,"(3X,I5,F14.6,4X,3(F8.4),F17.9)") &
              &  istring,wstring(istring), &
              &  xk(1,ind1),xk(2,ind1),xk(3,ind1),pdl_elec(istring)
      END DO
      WRITE( stdout,"(2X,76('-'))")
   END IF
   IF (noncolin) THEN
      WRITE( stdout,"(42X,'Average phase   : ',F17.9)") & 
        pdl_elec_up
   ELSE
      WRITE( stdout,"(42X,'ELECTRONIC PHASE: ',F17.9)") & 
        pdl_elec_tot
   ENDIF
   WRITE( stdout,"(2X,76('='))")
...
!  --- Information about total phase ---
   ...
   WRITE( stdout,"(31X,17('~'),/)")
   WRITE( stdout,"(26X,'Ionic Phase:',F14.8)") &
        pdl_ion_tot
   WRITE( stdout,"(21X,'Electronic Phase:',F14.8)") &
        pdl_elec_tot
   WRITE( stdout,"(26X,'TOTAL PHASE:',F14.8, ' MOD_TOT:',i2)") &
        pdl_tot,mod_tot
...
END SUBROUTINE c_phase_tpw

!==============================================================================!
!--------------------------------------------------------------------------
SUBROUTINE initialize_bp_pool(k_pool_base, nppstr, nkstot)
!--------------------------------------------------------------------------

USE mp_pools, ONLY : npool, my_pool_id

IMPLICIT NONE
INTEGER, INTENT(IN) :: nppstr, nkstot
INTEGER, INTENT(OUT) :: k_pool_base

INTEGER :: kunit, rest, nbase, nks
!
kunit=nppstr
!
nks    = kunit * ( nkstot / kunit / npool )
!
IF (nks == 0) CALL errore('initialize_bp_pool', 'Too many pools', 1)
!
...
RETURN
END SUBROUTINE initialize_bp_pool

!---------------------------------------------------------------------------
LOGICAL FUNCTION is_in_my_pool(ik, k_pool_base, nks)
!---------------------------------------------------------------------------
IMPLICIT NONE

INTEGER, INTENT(IN) :: ik, k_pool_base, nks
LOGICAL :: aux

aux=.FALSE.
aux= ((ik>k_pool_base).AND.(ik<= k_pool_base+nks))
is_in_my_pool=aux

RETURN
END FUNCTION is_in_my_pool
