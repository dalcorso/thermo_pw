...
!-----------------------------------------------------------------------
subroutine tra_write_matrix_tpw (alpha, adyn, u, nat)
  !-----------------------------------------------------------------------
  ...
  auxdyn=adyn
  CALL symdyn_munu_tpw (auxdyn, u, xq, s, invs, rtau, irt, at, bg, &
  ...
  do na = 1, nat
     do nb = 1, nat
        ...
        do i = 1, 3
           WRITE( stdout, '(6f12.6)') (auxdyn(3*(na-1)+i, 3*(nb-1)+j),j=1,3)
        enddo
     enddo
  enddo
  return
end subroutine tra_write_matrix_tpw
