...
!
!-----------------------------------------------------------------------
SUBROUTINE orthogonalize_tpw(dvpsi, evq, ikk, ikq, dpsi, npwq, dpsi_computed)
!------------------------------------------------------------------------
  !
  ...
  IF (ltetra .OR. lgauss) THEN
     ...
     !$acc end host_data
     !$acc update host(ps)
     CALL stop_clock ('ortho:1')
     CALL start_clock ('ortho:111')
     !
     DO ibnd = 1, nbnd_occ (ikk)
        !
        IF ( lgauss ) THEN
           !
           CALL start_clock ('ortho:11')
           wg1 = wgauss ((ef-et(ibnd,ikk)) / degauss, ngauss)
           w0g = w0gauss((ef-et(ibnd,ikk)) / degauss, ngauss) / degauss
           CALL stop_clock ('ortho:11')
           DO jbnd = 1, nbnd
              CALL start_clock ('ortho:12')
              ...
              IF (jbnd <= nbnd_occ (ikq) ) THEN
                 ...
                 ENDIF
              ENDIF
              CALL stop_clock ('ortho:12')
              CALL start_clock ('ortho:13')
              !
!              !$acc kernels present(ps)
              ps(jbnd,ibnd) = wwg * ps(jbnd,ibnd)
!              !$acc end kernels
              CALL stop_clock ('ortho:13')
              !
           ENDDO
           !
        ELSE
...
!              !$acc end kernels
        ...
        !
     END DO
     CALL start_clock ('ortho:14')
     !$acc update device(ps)
     CALL stop_clock ('ortho:14')
     !
     nbnd_eff=nbnd
     CALL stop_clock ('ortho:111')
     !   
  ELSE
     CALL start_clock ('ortho:1')
     !
     ...
     IF (okvan) then
        if (use_bgrp_in_hpsi .AND. .NOT. exx_is_active() .AND. nbnd_eff > 1) then
           ...
           if ( n_end >= n_start) then
              CALL calbec ( offload_type, npwq, vkb, evq(:,n_start:n_end), &
               becp, n_end - n_start + 1 )
           endif
        else
  ...
  RETURN
  !
END SUBROUTINE orthogonalize_tpw
