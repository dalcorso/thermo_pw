...
! Copyright (C) 2023 Andrea Dal Corso (for many k, many perturbations 
!                                      generalization)
...
!
!----------------------------------------------------------------------
SUBROUTINE cgsolve_all_many_k (ch_psi, cg_psi, e, d0psi, dpsi_d, h_diag,  &
...
#include<cgsolve_all_interf.f90>
  ...
  complex(DP) :: &
             dpsi_d (ndmx*npol, nbnd*nk*npe*nsolv), & ! output: the solution 
...
#if defined(__CUDA)
ATTRIBUTES(DEVICE) :: h_diag, d0psi, dpsi_d
#endif
  !
  ...
  complex(DP), allocatable :: g (:,:), t (:,:), h (:,:), hold (:,:), dpsi(:,:)
  !  the gradient of psi
  ...
  !$acc declare device_resident(hpsi, spsi, ps)
  ...
  allocate ( dpsi(ndmx*npol,my_nbnd*nk*npe*nsolv))
  ...
  !$acc enter data create(rho(1:my_nbnd*nk*npe*nsolv),a(1:my_nbnd*nk*npe*nsolv),c(1:my_nbnd*nk*npe*nsolv),eu(1:my_nbnd*nk*npe*nsolv),t(1:ndmx*npol,1:my_nbnd*nk*npe*nsolv),g(1:ndmx*npol,1:my_nbnd*nk*npe*nsolv),h(1:ndmx*npol,1:my_nbnd*nk*npe*nsolv),hold(1:ndmx*npol,1:my_nbnd*nk*npe*nsolv)) copyin(e(1:nbnd,1:nks),evq(1:ndmx*npol,1:nbnd),evc(1:ndmx*npol,1:nbnd),dpsi(1:ndmx*npol,1:my_nbnd*nk*npe*nsolv)) 
  ...
  DO ik1=1,nk
     ...
     DO isolv=1, nsolv
        ...
        DO ipert=1,npe
           id=ik1+(ipert-1)*nk + (isolv-1) * nk * npe
           st_=st(id)
           !$acc parallel loop present(eu,e)
           DO ibnd = 1, nbd
...
iterate:  do iter = 1, maxter
     ...
     if (iter == 1) then
        ...
        !$acc end kernels
        IF (okvan) THEN
           !$acc host_data use_device(dpsi,hpsi,spsi)
           ...
           !$acc end host_data 
        ELSE
           !$acc host_data use_device(dpsi,hpsi)
           ...
           ierr=cudaDeviceSynchronize() 
           !$acc end host_data 
           !$acc parallel loop present(dpsi,spsi)
           DO i=1,ndmx*npol
              spsi(i,1:my_nbnd*nk*npe*nsolv)=dpsi(i,1:my_nbnd*nk*npe*nsolv)
           ENDDO
        ENDIF

        !$acc host_data use_device(dpsi,g,hpsi,spsi,ps,eu)
     ...
     conv_d=conv
     lbndk_d=lbndk
     !$acc host_data use_device(h,g,rho)
     ...
     DO ik1=1,nk
        ...
        DO isolv=1, nsolv
           DO ipert=1,npe
              ...
              call mp_sum( rho(st_+1:st_+lbnd(id)), intra_bgrp_comm )
              do ibnd = n_end, n_start, -1 ; ibnd_ = ibnd - n_start + 1
                 IF (ibnd>nbnd_occ(ikk)) CYCLE
                 if (conv(st_+ibnd).eq.0) then
                    rho(st_+ibnd_)=rho(st_+lbnd(id))
                    lbnd(id) = lbnd(id) -1
                    anorm(id) = sqrt (rho (st_+ibnd_) )
...
!
           ...
           DO ipert=1,npe
              ...
              do ibnd = n_start, n_end ; ibnd_ = ibnd - n_start + 1
                 ...
                 if (conv (st_+ibnd) .eq.0) then
                    ...
                    lbndk(st_+ibnd)=lbnd(id)
                    if (iter.ne.1) then
                       dcgammak(st_+ibnd) = rho (st_+ibnd_) / rhoold (st_+ibnd_)
                    endif
                 endif
              enddo
           ENDDO
     ...
     lbndk_d=lbndk
     dcgammak_d=dcgammak
     !$acc host_data use_device(h,hold,eu,e)
...
!
    ...
    IF (okvan) THEN
       !$acc host_data use_device(hold,hpsi,spsi)
                 ...
                 if (conv (st_+ibnd) .eq.0) then
                    ...
                    dclambdak(st_+ lbnd(id)) = &
                          CMPLX( - a(st_+lbnd(id)) / c(st_+lbnd(id)), &
                                                           0.d0,kind=DP)
                    rhoold (st_+ibnd_) = rho (st_+ibnd_)
                 endif
              enddo
           ENDDO
        ENDDO
 ...
 conv_d=conv
 lbndk_d=lbndk
 !$acc host_data use_device(dpsi, g, h, hold, t)
 ...
 !$acc end host_data
           ...
           DO ipert=1,npe
              ...
              do ibnd = n_start, n_end ; ibnd_ = ibnd - n_start + 1
                 ...
                 if (conv (st_+ibnd) .eq.0) then
                    ...
                    !
                    !$acc host_data use_device(h,dpsi)
  ...
  enddo iterate
  !$acc update host(dpsi)
  !$acc exit data delete(rho,evq,evc,a,c,g,h,hold,t,eu,e,dpsi) 

  ! deallocate workspace not needed anymore
  deallocate (eu) ; deallocate (rho, rhoold) ; deallocate (a,c) ; deallocate (g, t, h, hold) ; deallocate(hpsi,spsi,ps)
