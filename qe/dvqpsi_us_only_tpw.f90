...
!
!----------------------------------------------------------------------
subroutine dvqpsi_us_only_tpw (ik, id, uact, becp1, alphap)
  !----------------------------------------------------------------------
  ...
  USE many_k_ph_mod,  ONLY : ps1k, ps2k
                    ...
                    do ipol = 1, 3
                       if ( abs (uact (mu + 1) ) + &
                          ...
                          IF (noncolin) THEN
                             ...
                             END DO
                          ELSE
                             ps1k (ikb, ibnd, id) = ps1k (ikb, ibnd, id) +      &
                                        deff(ih, jh, na) *            &
                                alphap(ipol, ik)%k(jkb, ibnd) * uact (mu + ipol)
                             ps2k (ikb, ibnd, ipol, id) = &
                                     ps2k (ikb, ibnd, ipol, id) +&
  ...
  if (nkb.gt.0) then
     if (noncolin) then
        ...
        call zgemm ('N', 'N', npwq, nbnd, nkb, &
         (1.d0, 0.d0) , vkb, npwx, ps1k(1,1,id), nkb, (1.d0, 0.d0) , dvpsi, npwx)
     end if
  end if
  !
  ...
  do ikb = 1, nkb
     do ipol = 1, 3
        ...
        ELSE
           do ibnd = 1, nbnd
              ok = ok.or. (abs (ps2k (ikb, ibnd, ipol, id) ) .gt.eps)
           enddo
        ENDIF
        if (ok) then
           do ig = 1, npwq
...
!                                                         dvpsi(1+npwx,ibnd),1)
  ...
  return
end subroutine dvqpsi_us_only_tpw
