...
!
!-----------------------------------------------------------------------
subroutine irreducible_BZ_tpw (nrot, s, nsym, minus_q, magnetic_sym, at, bg, &
                           npk, nks, xk, wk, t_rev)
  !-----------------------------------------------------------------------
  !
  ...
  return
  !
end subroutine irreducible_BZ_tpw 
!
!-----------------------------------------------------------------------
subroutine irrek_tpw (at, bg, nrot, invs, nsym, irg, minus_q, npk, &
                  nks, xk, wk, t_rev, diago_bands, isym_bands, ik_origin)
  !-----------------------------------------------------------------------
  !
  ...
  LOGICAL, INTENT(OUT) :: diago_bands(npk)
  INTEGER, INTENT(OUT) :: isym_bands(npk), ik_origin(npk)
  
  ! .true. if symmetries q = -q+G are acceptable
  !
  !    here the local variables
  !
  integer :: nks0, jk, kpol, irot, jrot, ncos, jc, ic, isym, icos, ik
  ...
  DO ik=1,nks
!
!   Presently it is too difficult to read the wavefunctions calculated
!   by the pw.x run. The list of G vectors in each processor is changing
!   from a run to another, and all the fft indexes change. So for the 
!   moment we recompute the bands in these k points. I belive this is
!   a terrific feature of the present version of QE.
!
    diago_bands(ik)=.TRUE.
    ik_origin(ik)=ik
    isym_bands(ik)=1
...
100     continue
     ...
     do ic = 2, ncos
        ...
        if (w (ic) /= 0.d0) then
           ...
           enddo
           diago_bands(nks)=.FALSE.
           ik_origin(nks)=jk
           isym_bands(nks)=irg(irot)
        endif
     enddo
  ...
  !
  return
end subroutine irrek_tpw
!-----------------------------------------------------------------------
subroutine irrek_nc_tpw (at, bg, nrot, invs, nsym, irg, npk, &
                  nks, xk, wk, t_rev, diago_bands, isym_bands, ik_origin)
  !-----------------------------------------------------------------------
  !
  !  Given a set of special points in the Irreducible Wedge of some
  ...
  INTEGER :: isym_bands_new(npk), ik_origin_new(npk)
...
!
     ...
     xk(:,iks)=xk_cart(:)
     wk(iks)=wk_new(ik)
     diago_bands(iks)=.FALSE.
     isym_bands(iks)=isym_bands_new(ik)
     ik_origin(iks)=ik_origin_new(ik)
200  CONTINUE
  ENDDO
