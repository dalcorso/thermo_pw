...
SUBROUTINE new_polarizc ( iu )
  ...
  ! local variables
  !
  INTEGER :: ibnd, ipol, jpol, nrec, ik, ierr, npw, isolv, nsolv
  ! counter on polarizations
  ! counter on records
  ! counter on k points
  REAL(KIND=DP) :: wei, weight, repsilon(3,3), iepsilon(3,3)
  COMPLEX(KIND=DP) :: cepsilon(3,3), alpha(3,3), w
...
!
     ...
     DO isolv=1, nsolv
        DO ipol = 1, 3
           ...
           DO jpol = 1, 3
              nrec = (jpol - 1) * nksq + ik
              IF (isolv==2) nrec = nrec + 3 * nksq
              CALL get_buffer(dpsi, lrdwf, iudwf, nrec)
              DO ibnd = 1, nbnd_occ (ik)
                 !
                 !  this is  <DeltaV*psi(E)|DeltaPsi(E, w)>
                 !
                 cepsilon(ipol,jpol)=cepsilon(ipol,jpol)-2.d0*wei* &
                      ZDOTC (npw, dvpsi (1, ibnd), 1, dpsi (1, ibnd), 1) 
                 IF (noncolin) &
                    cepsilon(ipol,jpol)=cepsilon(ipol,jpol)-2.d0*wei* &
                      ZDOTC (npw, dvpsi (1+npwx, ibnd), 1, &
                                              dpsi (1+npwx, ibnd), 1) 
              ENDDO  ! nbnd
           ENDDO    ! jpol
        ENDDO      ! ipol
