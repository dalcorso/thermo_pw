...
!=----------------------------------------------------------------------------=!
  ...
  !
  !-----------------------------------------------------------------------
SUBROUTINE readpp_tpw ( input_dft, printout, ecutwfc_pp, ecutrho_pp )
  !-----------------------------------------------------------------------
  !
  ...
  USE upf_module_tpw,   ONLY: read_upf_tpw
  ...
  do nt = 1, ntyp
     ...
     END IF
     !
     CALL  read_upf_tpw(upf(nt), rgrid(nt), isupf, filename = file_pseudo, xml_only = .TRUE. )
     !
     ...
     IF ( isupf .GT. 0 ) THEN
       ...
       CALL read_upf_tpw( upf(nt), rgrid(nt), isupf, UNIT = iunps) 
     END IF  
     upf(nt)%is_gth=.false.
...
END MODULE read_pseudo_mod_tpw
