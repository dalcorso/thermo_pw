...
!
!-----------------------------------------------------------------------
SUBROUTINE irreducible_BZ_old( nrot, s, nsym, minus_q, magnetic_sym, at, bg, &
                           npk, nks, xk, wk, t_rev )
  !-----------------------------------------------------------------------
  ...
  IF ( magnetic_sym ) THEN
     CALL irrek_nc_old( at, bg, nrot, invs, nsym, irg, npk, nks, xk, &
                    wk, t_rev )
  ELSE
     sym(1:nsym) = .TRUE.
     sym(nsym+1:) = .FALSE.
     ...
     !     of the crystal.
     !
     CALL irrek_old( at, bg, nrot, invs, nsym, irg, minus_q, npk, nks, xk, &
                 wk, t_rev )
  ENDIF
  !
  RETURN
  !
END SUBROUTINE irreducible_BZ_old 
!
!
!-----------------------------------------------------------------------
SUBROUTINE irrek_old( at, bg, nrot, invs, nsym, irg, minus_q, npk, &
                  nks, xk, wk, t_rev )
  !-----------------------------------------------------------------------
...
100     CONTINUE
  ...
  RETURN
  !
END SUBROUTINE irrek_old
!
!
!-----------------------------------------------------------------------
SUBROUTINE irrek_nc_old( at, bg, nrot, invs, nsym, irg, npk, &
                     nks, xk, wk, t_rev )
  !-----------------------------------------------------------------------
  !! Given a set of special points in the Irreducible Wedge of some
  !! group, finds the equivalent special points in the IW of one of
