...
!
!---------------------------------------------------------------------
subroutine set_irr_tpw (xq, u, npert, nirr, eigen) 
!---------------------------------------------------------------------
!
...
!
     ...
     IF (search_sym.AND.nspin_mag/=4) THEN
        CALL find_mode_sym_tpw (u, eigen, tau, nat, nsymq, s, sr, irt, xq,    &
        ...
        DO imode=1,3*nat-1
           DO jmode = imode+1, 3*nat
              IF ((num_rap_mode(imode) /= num_rap_mode(jmode)).AND.  &
                  (is_degenerate_mode(eigen(imode),eigen(jmode)))) THEN
...
! initialization
     ...
     npert (1) = 1
     do imode = 2, 3 * nat
        if (is_degenerate_mode(eigen(imode),eigen(imode-1))) then
...
400 continue
  ...
  return
end subroutine set_irr_tpw

!
!-----------------------------------------------------------------------
subroutine ccryst_to_cart (nvec, vec, trmat, iflag)
  !-----------------------------------------------------------------------
  !
  ...
  COMPLEX(DP), intent(inout) :: vec (3, nvec)
  ...
  !
  COMPLEX(DP) :: veu(3,nvec), trmatc(3,3)
  !
  veu=vec
  trmatc=CMPLX(trmat,0.0_DP)
  IF (iflag==1) THEN
!     DO kpol = 1, 3
!        vec(kpol,:) = trmat (kpol, 1) * veu (1, :) + trmat (kpol, 2) &
!                 * veu (2, :) + trmat (kpol, 3) * veu (3, :)
!     ENDDO
     CALL zgemm('N','N',3,nvec,3,(1.0_DP,0.0_DP),trmatc,3,veu,&
                                           3,(0.0_DP,0.0_DP),vec,3)
  ELSE
!     DO kpol = 1, 3
!        vec (kpol,:) = trmat (1, kpol) * veu (1, :) + trmat (2, kpol) &
!             * veu (2, :) + trmat (3, kpol) * veu (3, :)
!     ENDDO
     CALL zgemm('T','N',3,nvec,3,(1.0_DP,0.0_DP),trmatc,3,veu,&
                                           3,(0.0_DP,0.0_DP),vec,3)
  ENDIF
 
  return
end subroutine ccryst_to_cart
!
!-----------------------------------------------------------------------
...
LOGICAL FUNCTION is_degenerate_mode(a, b)
...
IF (ABS(a)<1.D-9.AND.ABS(b)<1.D-9.AND.ABS(a-b)<1.D-9) THEN
   is_degenerate_mode=.TRUE.
ELSE
   is_degenerate_mode = (ABS(a-b)/(ABS(a)+ABS(b)) < 1.d-4)
ENDIF
RETURN
END FUNCTION is_degenerate_mode
