  ...
  DO iper = 1, 3
     DO is = 1, nspin_mag
        DO ir3 = 1, dfftp%my_nr3p
           ...
           CALL zcopy (nxyp, &
              ddvtosym(ioff_tg+1, is, iper), 1, dvtosym (ioff+1, is, iper), 1)
        ENDDO
     ENDDO
  ENDDO
...
#else
  CALL syme_fpol (dvtosym)
#endif

  RETURN

END SUBROUTINE psyme_fpol
