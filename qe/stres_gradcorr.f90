...
!
!----------------------------------------------------------------------------
subroutine stres_gradcorr_tpw( rho, rhog, rho_core, rhog_core, kedtau, nspin, &
  ...
  REAL(DP), ALLOCATABLE :: rhoout(:,:), segni(:)
  ...
  IF ( nspin == 4 .AND. domag ) THEN
     ...
     DO is = 1, nspin0
        ...
        !
        rhogout(:,is) = psic(dfft%nl(:))
        !
     END DO
  ELSE
     !
     rhoout(:,1:nspin0)  = rho(:,1:nspin0)
     rhogout(:,1:nspin0) = rhog(:,1:nspin0)
     !
  ENDIF
  ...
  !
  IF ( nspin0 == 1 ) THEN
     rhogaux(:,1)  = rhogout(:,1) + rhog_core(:)
  ELSE
     rhogaux(:,1)  = ( rhogout(:,1) + rhogout(:,2) + rhog_core(:) ) / 2.0_DP
     rhogaux(:,2)  = ( rhogout(:,1) - rhogout(:,2) + rhog_core(:) ) / 2.0_DP
  ENDIF
  !
  DO is = 1, nspin0
  ...
  IF ( nspin0 == 1 ) THEN
     rhoaux(:,1)  = rhoout(:,1) + rho_core(:)
  ELSE
     rhoaux(:,1)  = ( rhoout(:,1) + rhoout(:,2) + rho_core(:) ) / 2.0_DP
     rhoaux(:,2)  = ( rhoout(:,1) - rhoout(:,2) + rho_core(:) ) / 2.0_DP
  ENDIF
  !
  if (nspin0.eq.1) then
     !
  ...
  RETURN
END SUBROUTINE stres_gradcorr_tpw
