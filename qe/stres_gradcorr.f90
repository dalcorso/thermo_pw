...
subroutine stres_gradcorr_tpw( rho, rhog, rho_core, rhog_core, kedtau, nspin, &
  ...
  real(DP), allocatable :: grho (:,:,:), rhoout(:,:), segni(:)
  ...
  ENDIF
  !
  !    calculate the gradient of rho+rhocore 
  !
  DO is = 1, nspin0
     !
     rhoout(:,is)  = fac * rho_core(:)  + rhoout(:,is)
     rhogout(:,is) = fac * rhog_core(:) + rhogout(:,is)
     !
     CALL fft_gradient_g2r( dfft, rhogout(1,is), g, grho(1,1,is) )
     !
  END DO
  !
  if (nspin0.eq.1) then
     ...
     do k = 1, nrxx
        ...
        if (abs (rhoout (k, 1) ) .gt.epsr.and.grho2 (1) .gt.epsg) then
           ...
           if ( dft_is_meta() ) then
              ...
              call tau_xc (rhoout(k,1), grho2(1),kedtau(k,1), sx, sc, v1x, &
                                                      v2x,v3x,v1c,v2c,v3c)
              kedtau(k,1) = kedtau(k,1) * e2
              !
           else
