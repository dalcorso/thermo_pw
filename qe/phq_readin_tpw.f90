...
SUBROUTINE phq_readin_tpw()
  ...
  !
  IF (meta_ionode) THEN
     READ( qestdin, INPUTPH, ERR=30, IOSTAT = ios )
     !
...
30 CONTINUE
  ...
  IF (niter_ph < 0 .OR. niter_ph > maxter) CALL errore ('phq_readin', &
       ' Wrong niter_ph ', 1)
  IF (nmix_ph.LT.1.OR.nmix_ph.GT.8) CALL errore ('phq_readin', ' Wrong &
       &nmix_ph ', 1)
...
#if defined(__CUDA)
  IF (llanczos) CALL errore('phq_readin','Lanczos algorithm &
...
!                                                  &not running on GPU',1)
  ...
  IF (.NOT.ldisp .AND. .NOT. qplot) THEN
     ...
     IF ( (.NOT.lgamma.AND..NOT.fpol) .AND. (epsil.OR.zue.OR.lraman.OR.elop) )&
  ...
  IF ( fpol ) THEN
     ...
     IF (meta_ionode) THEN
        IF ( TRIM(card) == 'FREQUENCIES' .OR. &
             ...
             TRIM(card) == 'Frequencies' ) THEN
           IF (freq_line) THEN
              READ (qestdin, *, iostat = ios) fru(1), fiu(1)
              IF (ABS(delta_freq)/=0.0_DP) THEN
                 fru(nfs) = fru(1) + DREAL(delta_freq) * (nfs - 1)
                 fiu(nfs) = fiu(1) + DIMAG(delta_freq) * (nfs - 1)
              ELSE
                 READ (qestdin, *, iostat = ios) fru(nfs), fiu(nfs)
              END IF    
           ...
           ELSE   
              DO i = 1, nfs
                 READ (qestdin, *, iostat = ios) fru(i), fiu(i)
              END DO
           ENDIF
           IF (lfreq_ev) THEN
              fru=fru/rytoev
