...
!
!----------------------------------------------------------------------
subroutine addusdbec_tpw (ik, wgt, dpsi, dbecsum, id)
  !----------------------------------------------------------------------
  !
...
!  CALL calbec (npwq, vkb, dpsi, dbecq)
  dbecq(1:nkb,1:nbnd_occ(ikk))=dbecq_d(1:nkb,1:nbnd_occ(ikk),id)
  !
...
!!                    dbecsum (ijh, na) = dbecsum (ijh, na) + &
!!                   wgt * ( CONJG(becpk_d(ikb,ibnd,id)) * dbecq_d(ikb,ibnd,id) )
                 enddo
                 do jh = ih + 1, nh (nt)
                    ijh=ijtoh(ih,jh,nt)
                    jkb = ijkb0 + jh
                    do ibnd = startb, lastb
!!                      dbecsum (ijh, na) = dbecsum (ijh, na) + &
!!                   wgt*( CONJG(becpk_d(ikb,ibnd,id))*dbecq_d(jkb,ibnd,id) + &
!!                         CONJG(becpk_d(jkb,ibnd,id))*dbecq_d(ikb,ibnd,id) )
                      dbecsum (ijh, na) = dbecsum (ijh, na) + &
