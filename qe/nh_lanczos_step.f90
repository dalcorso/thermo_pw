...
!
SUBROUTINE nh_lanczos_step(iter, flag)
!
...
!   there must be A evc1 and in evc1_new(:,:,:,2) A^T evc1, where A is 
!   the linear operator to tridiagonalize.
...
!   
!   This version of the routine is only for non-hermitean matrices.
!
    USE kinds,        ONLY : DP
    ...
    CALL lr_apply_s(evc1(:,:,:,2), sevc1(:,:,:,1))
    !
...
!    WRITE(stdout,'(5X,"beta (",i8.8,")=",f15.6)') iter, beta
    ...
    !
    CALL ZSCAL(size_evc1,CMPLX(1.0d0/dgamma,0.0d0,kind=dp),evc1(1,1,1,2),1)
    CALL ZSCAL(size_evc1,CMPLX(1.0d0/dgamma,0.0d0,kind=dp),evc1_new(1,1,1,2),1)
    !
...
!       WRITE(stdout,'(5x,"z1= ",2(1x,e22.15))') real(zeta),aimag(zeta)
    ...
    CALL ZAXPY(size_evc1,-CMPLX(beta,0.0d0,KIND=DP),evc1_old(1,1,1,2),1,&
                                                       evc1_new(1,1,1,2),1)
...
!
    ...
    !
    CALL ZCOPY(size_evc1*2,evc1,1,evc1_old,1) ! evc1_old = evc1
    CALL ZCOPY(size_evc1*2,evc1_new,1,evc1,1) ! evc1 = evc1_new
    !
    CALL stop_clock('lanczos_step')
    !
    RETURN
    !
END SUBROUTINE nh_lanczos_step
