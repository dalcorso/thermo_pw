...
\begin{verbatim}
what         A   B   C   D   E    F  H0  G1    H    I   L   G2   M  N  O  P  Q
...
with a quadratic polynomial, it compute it, to find its gradient or 
the hessian in a given point and to find the extremum of the quadratic 
polynomial. 
...
a given point and to find the extremum of the quartic polynomial
closest to a given input point. 

\item
\texttt{polyfit.f90} contains routines to deal with polynomial
interpolation of arbitrary degree but for a single variable. Presently
it contains only a routine to find the polynomial that interpolates
a set of data.
