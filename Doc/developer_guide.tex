...
\texttt{What} that does not require different geometries, such as
\texttt{what='scf\_ke'} create an \texttt{outdir} directory called
\texttt{outdir/ke\#} for each different task, where \texttt{\#} is the
task number. Similarly \texttt{what='scf\_ke'} uses outdir directories
...
\thermo\ does not uses this feature and sets always the wavefunction
...
representation for each band. The format of these files is readable
...
by the \texttt{plotband.f90} code of Quantum ESPRESSO. The numbers in
...
This file contains the following quantities
isostress (or constant pressure) heat capacity and the difference 
between the two.
\begin{verbatim}
#   T       C_v(T)        C_p(T)        (C_p - C_v)(T)    
\end{verbatim}
the temperature, the isothermal heat capacity calculated 
...
\begin{figure*}[ht]
  \begin{tabular}[t]{ll}
    \hspace{-1cm}\includegraphics[height=0.80\textwidth]{flowmaster.pdf}
    &\hspace{-1cm}\includegraphics[height=0.80\textwidth]{flowslave.pdf}\\
  \end{tabular}
