...
\item 
\texttt{therm\_files/flevdat} (default name \texttt{output\_ev.dat})
...
\item 
\texttt{therm\_files/flevdat(\_mur)} (default name \texttt{output\_ev.dat\_mur})
...
\item 
\texttt{therm\_files/flevdat(.ev.out)} 
...
with more digits and in a \texttt{XML} format.
...
The file contains for each {\bf q} vector its weight and the frequencies,
...
\texttt{output\_dos.dat.g\#}). For each geometry for which a phonon dispersion
is calculated a phonon density of states is also calculated when
\texttt{ltherm\_dos=.TRUE.}. The phonon dos is written in this file 
with the form:
\begin{verbatim}
#   Frequency       dos
\end{verbatim}
...
only when \texttt{lmurn=.TRUE.} and contains:
\begin{verbatim}
   T     gamma(T)    C_v (T)    (C_p - C_v)(T)  (B_S - B_T) (T) 
\end{verbatim}
The temperature, the average Gr\"uneisen parameter, the isochoric heat
capacity computed from interpolation of the volume dependent isochoric heat
...
#   T        beta      gamma    C_v (T) 
\end{verbatim}
the temperature, the volume thermal expansion calculated from mode 
Gr\"uneisen parameters, the average Gr\"uneisein parameter that results
from the new $\beta$ but keeping all the other quantities as calculated
from the free energy derivatives. The isochoric heat capacity is calculated
by interpolating the isochoric heat capacity.
...
These files contains the Gr\"uneisen parameters along the path.
