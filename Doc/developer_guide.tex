%!
%! Copyright (C) 2015-2025 Andrea Dal Corso 
...
\item 
I0) \texttt{lpiezotqha=.TRUE.} computes the piezoelectric tensor at $T=0$
from the polarization induced by a strain.
...
\item
R) Interpolation of the temperature dependent elastic constants within the
`quasi-static' or `quasi-harmonic' approximation. Ionic positions can be
treated within the 

\item
S) Interpolation of the temperature dependent piezoelectric tensor within the
`quasi-harmonic' ZSISA or (full free energy minimization) FFEM approximation.

\end{itemize}

If \texttt{lectqha=.TRUE.} the code will do:
\begin{itemize}
\item
...
If \texttt{lpiezotqha=.TRUE.} the code will do:
\begin{itemize}
\item
T) Calculation of the temperature dependent piezoelectric tensor within the
`fixed-geometry quasi-harmonic' approximation (within ZSISA or FFEM).
\end{itemize}

If \texttt{lpyrotqha=.TRUE.} the code will do:
\begin{itemize}
\item
T) Calculation of the temperature dependent pyroelectric tensor within the
`fixed-geometry quasi-harmonic' approximation (only within FFEM).
\end{itemize}
...
\begin{footnotesize}
\begin{verbatim}
what         A  B  C  D  E  F  H0  G1  H  I  L  G2  M  N  O  P  Q  R  S  T  U
...
restart         : a directory where each task creates a file with the total
                  ...
                  If the task computes the Berry phase, both the polarization
                  and the Berry phase of each geometry are saved here.
                  This directory contains also files with the geometry
                  of each task: bravais lattice, crystal parameters and
                  atomic positions. If the bravais lattece is ibrav=0 
                  this file contains also the primitive Bravais lattice 
                  vectors at. Atomic positions files with extension mlc 
                  correspond to equilibrium geometries, while extension 
                  elc correspond to distorted geometries.
...
anhar_files     : a directory that contains the files with the anharmonic
                  ...
                  The 'quasi-static, 'fixed geometry FFEM' piezoelectric
                  tensor files are saved here.

elastic_constants : a directory that contains the elastic constants files
                  at T=0 K. This directory contains also the piezoelectric
                  tensor file at T=0 K when this is calculated.
                  It can contain also files with the dielectric constant
                  and the Born effective charges for each geometry when 
                  they are calculated.
\end{verbatim}
\end{footnotesize}
...
Debye approximation,
\begin{verbatim}
manage_piezo_tensor.f90  
\end{verbatim}
coordinates the calculation of the piezoelectric tensor, both proper and
improper, its decomposition into a clamped ions and a relaxed ion term, and
the calculation of the polarization of the undistorted geometry,
\begin{verbatim}
manage_polarization.f90  
\end{verbatim}
coordinates the calculation of the spontaneous polarization
\begin{verbatim}
...
\begin{verbatim}
manage_piezo_tensor_qha.f90
\end{verbatim}
coordinates the calculation of the temperature dependent piezoelectric tensor
within the FFEM,
\begin{verbatim}
manage_polarization_qha.f90  
\end{verbatim}
coordinates the calculation of the pyroelectric tensor with FFEM,
\begin{verbatim}
...
\item
\texttt{dielectric.f90} contains routines to calculate the static dielectric 
constant $\epsilon_0$ and routines to save on files the dielectric constant
...
\item
\texttt{polarization.f90} contains routine to deal with the Berry phase
polarization. Presently only files to read and write it on file.
...
The command \texttt{make join\_qe} substitutes four files of the QE
distribution: \texttt{Makefile} in the main QE directory, 
\texttt{install/makedeps.sh}, \texttt{install/}
\texttt{plugins\_makefile} and \texttt{CMakeLists.txt}. 
...
to give the command \texttt{make join\_qe} before compiling QE.
Some files contains parameters which are increased to improve the 
working of \texttt{thermo\_pw}. For instance, we increase the maximum 
number of ${\bf k}$ points, the maximum number of allocatable clocks, etc.
