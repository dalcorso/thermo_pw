...
\end{equation}
The sum over ${\bf k}$ is limited to the IBZ and the sum over $l$ is over the $N_{\bf k}$ symmetries needed to obtain all the points of the star of ${\bf k}$ that belong to the mesh from the 
...
a weight $w_{\bf k}$ (see next section for a more precise definition) 
...
\section{The special point technique}

Suppose we have a function of the reciprocal lattice vector $g({\bf k})$,
periodic with the periodicity of the reciprocal lattice vectors
${\bf G}$. We can expand this function in Fourier series:
\begin{equation}
g({\bf k})= g_0 + \sum_\mu g_\mu e^{i{\bf k}\cdot {\bf R}_\mu},
\end{equation}
${\bf R}_\mu$ are the direct lattice vectors (where ${\bf R}_\mu=0$ has 
been separated from the sum). Suppose $G$ is the point group of the Bravais
lattice and $h$ the number of symmetry elements of 
$G=\left\{S_1,...,S_h\right\}$. We can consider the symmetric
function
\begin{equation}
f({\bf k}) = {1\over h} \sum_{i=1}^h g(S_i {\bf k})
\end{equation}
We have 
\begin{equation}
f({\bf k}) = f_0 + \sum_\mu g_\mu A_\mu ({\bf k})
\end{equation}
where $f_0=g_0$ and 
\begin{equation}
A_\mu ({\bf k}) = {1\over h} \sum_{i=1}^h e^{i S_i {\bf k} \cdot {\bf R}_\mu} =
{1\over h} \sum_{i=1}^h e^{i {\bf k} \cdot S_i^{-1}{\bf R}_\mu}.
\end{equation}
The integral over the Brillouin zone of $A_\mu ({\bf k})=0$ for all $\mu$ 
except $\mu=0$ hence the average over the Brillouin zone of $f({\bf k})$ and
of $g(\bf k)$ coincide:
\begin{equation}
{\Omega \over (2\pi)^3} \int_{BZ} d^3k g({\bf k}) = {\Omega \over (2\pi)^3} \int_{BZ} d^3k f({\bf k}) = f_0.
\end{equation}
The integral is discretized with a uniform mesh of $N_{\bf k}=N_1\times N_2 
\times N_3$ points (see the routine \texttt{kpoint\_grid}) and therefore one
calculates the sum
\begin{equation}
{\Omega \over (2\pi)^3} \int_{BZ} d^3k f({\bf k}) =
{1\over N_{\bf k}h} \sum_{j=1}^{N_{\bf k}} \sum_{i=1}^h g(S_i {\bf k}_j).
\label{sum}
\end{equation}
The mesh is usually chosen in such a way to set to zero the largest number
of functions $A_{\mu}({\bf k})$. In practice however one chooses a uniform
mesh. 
All the points ${\bf k}_j$ that belong to the same star of ${\bf k}$
give the same contribution to the sum (Eq.~\ref{sum}). So we can take 
only one point
in the irreducible Brillouin zone (reduced with the Bravais lattice point
group) ($IBZ_{L}$) and put a weight equal to the number of {\bf k} point
of the star of ${\bf k}_j$ that belong to the original mesh. We obtain
\begin{equation}
{\Omega \over (2\pi)^3} \int_{BZ} d^3k f({\bf k}) =
{1\over N_{\bf k}h} \sum_{j\in IBZ_L} w_j \sum_{i=1}^h g(S_i {\bf k}_j) 
={1\over h} \sum_{j\in IBZ_L} \tilde w_j \sum_{i=1}^h g(S_i {\bf k}_j),
\end{equation}
where in the last term $\tilde w_j = w_j / N_{\bf k}$ and the sum over
all weights is equal to one.

We note that the function $g({\bf k})$ has usually the symmetry of
the solid, or as in the case of the charge density one can relate the values
of $g(S_i {\bf k}_j)$ at one point ${\bf r}$ with those of a different
point ${\bf r}'$, but only for the operations of the point group of the 
solid which is usually a subgroup of $G$. We call it $H$, with elements 
$S_p$ with $p=1,...,k$. 
It is convenient then to divide the group $G$ into $N_c=h/k$ left cosets 
of $H$ (see the
routine \texttt{coset} for a definition):
\begin{equation}
G= H + S_i H + S_l H +...+ S_{N_c} H.
\end{equation}
When we apply the operations of $H$ to a ${\bf k}_j$ point we obtain the
star of ${\bf k}_j$ (according to the group $H$) which is usually smaller than
the star of ${\bf k}_j$ for the group $G$.
So he have:
\begin{equation}
{\Omega \over (2\pi)^3} \int_{BZ} d^3k f({\bf k}) =
{1\over h} \sum_{j\in IBZ_L} \tilde w_j \sum_{l=1}^{N_c}\sum_{p=1}^k g(S_l S_p {\bf k}_j).
\end{equation}
Now when we apply all the operations of $H$ to $S_l {\bf k}_j$ we obtain
the star of $S_l {\bf k}_j$ for the group $H$. All the points of this star 
give the same contribution to this sum. Therefore, we can take only 
one point ${\bf k}_m$ in the irreducible Brillouin zone (reduced with 
the solid point group)
($IBZ_S$) and put a weight equal to $\tilde w_j$ multiplied by the number 
of times we find the same star varying $S_l$ among the $N_c$ cosets $w(m)$.
\begin{equation}
{\Omega \over (2\pi)^3} \int_{BZ} d^3k f({\bf k}) =
{1\over h} \sum_{m\in IBZ_S} \tilde w_m \sum_{p=1}^k g(S_p {\bf k}_m) =
{1\over k} \sum_{p=1}^k \sum_{m\in IBZ_S} \bar w_m g(S_p {\bf k}_m),
\end{equation}
where $\tilde w_m = \tilde w_j w(m)$ and $\bar w_m = {\tilde w_m \over N_c}$.
This equation coincides with Eq.~\ref{charge_density} taking 
\begin{equation}
g({\bf k})= 2 |\psi_{{\bf k}, v} ({\bf r}) |^2,
\end{equation}
and defines how one has to calculate the weights that appear in that equation.
...
The force acting on atom $s$ is defined as minus the derivatives of the 
total energy with respect to the same displacement of all the atoms
$I=\{\mu,s\}$ as $\mu$ varies in all the Bravais lattice vectors and
$s$ is given:
\begin{equation}
{\bf F}_{s,\alpha} = -{1\over N} \sum_\mu {d E_{tot} \over d 
{\bf u}_{\mu, s,\alpha}},
\end{equation}
where $N$ is the number of unit cells of the solid.
...
If we do an operation of the solid space group
...
\label{rotated_disp}
\end{equation}
the energy do not change since we have rigidly rotated the solid and its
displacements and we can write:
\begin{equation}
...
{d E_{tot} \over d {\bf u}'_{{\bar \mu},{\bar s},\beta}},
\end{equation}
where we used the fact that $S_{i,\alpha,\beta}$ is an orthogonal matrix and
only ${\bf u}'_{\bar I}$ depends on ${\bf u}_{I}$.
...
$S_i$ we obtain the symmetrized forces:
\begin{equation}
...
\begin{eqnarray}
D_{s,\alpha,s',\beta}({\bf q}) =
{1 \over N^3 \sqrt{M_s M_s'}} \sum_{\mu,\bar \mu, \nu,\bar \nu} \sum_{\gamma,\delta=1}^3  S^{-1}_{\alpha,\gamma} S^{-1}_{\beta,\delta}
...
\begin{equation}
D_{s,\alpha,s',\beta}({\bf q}) =
\sum_{\gamma,\delta=1}^3  S^{-1}_{\alpha,\gamma} S^{-1}_{\beta,\delta}
...
and dividing by $N_S$. We have:
\begin{equation}
\epsilon_{\alpha,\beta}= {1 \over N_S} \sum_{i=1}^{N_S}
\sum_{\gamma,\delta=1}^3 S^{-1}_{i,\alpha,\gamma}
S^{-1}_{i,\beta,\delta}
\epsilon_{\gamma,\delta}.
\label{symepsilon}
...
\begin{equation}
qZ^*_{s,\alpha,\beta}= {1\over N} \sum_\mu {\partial E_{tot} \over \partial {\bf u}_{\mu,s,\alpha} \partial {\bf E}_\beta}.
\end{equation}
...
=E_{tot}(\{({\bf R}_{\bar I}+{\bf u}'_{\bar I})\},{\bf E}'),
\end{equation}
and performing the derivatives we have:
\begin{equation}
{\partial E_{tot}(\{{\bf R}_I+{\bf u}_I\},{\bf E}) \over  \partial {\bf u}_{\mu,s,\alpha} \partial {\bf E}_\beta} = \sum_{\gamma,\delta} S^{-1}_{\alpha,\gamma} S^{-1}_{\beta,\delta} {\partial 
E_{tot}(\{({\bf R}_{\bar I}+{\bf u}'_{\bar I})\},{\bf E}')
\over \partial {\bf u}'_{\bar \mu,\bar s,\gamma} \partial {\bf E}'_\delta},
\end{equation}
or equivalently
\begin{equation}
Z^*_{s,\alpha,\beta}
...
In this chapter we present the modifications of the previous considerations needed to deal with spinor wavefunctions.
...
\item
\texttt{at(3,3)} contains the principal lattice vectors ${\bf a}_{\alpha,i}$ with this order for the two indexes (the first is Cartesian, the second indicates the vector). 
...
We write the expression for stress:
\begin{equation}
\sigma_{\alpha,\beta}={1 \over N_S} \sum_{i=1}^{N_S}
...
\section{Appendix: Coset}

Let's give a few definitions. Given a group $G=\{g_1,g_2,...,g_h\}$ with 
$h$ elements, we call a subgroup $H=\{h_1,...,h_k\}$ a subset of the 
elements of the group $G$ that are still a group. If $k$ is the number 
of elements of $H$, one can show that $N_c=h/k$ is an integer. 
Given $H$, we can define a left coset of $G$ by choosing an element 
of $G$ that does not belong to $H$, let's say $g_i$, and forming all the
products $g_i H = \{ g_i h_1,..., g_i h_k\}$. This set is composed by
$k$ elements of $G$ and is called a left coset. 
$H$ and $g_i H$ have no common elements, so we can write
$G=H + g_i H + ...$. If $h/k=2$ then $G=H+g_i H$ and we have finished, 
otherwise we can chose a $g_j$ not contained in $H+g_i H$ and form 
another left coset $g_j H$. We continue in this way for $N_c$ times
and finally write $G=H+g_i H + g_j H +...$. 

The routine coset gives and integer array \texttt{irg} of size $h$ (called
\texttt{nrot} inside the routine). The first $k$ elements (called 
\texttt{nsym}) of \texttt{irg} give the elements of $H$, then the next
$k$ elements are the indices of the first coset $g_i H$, the next
$k$ elements are the indices of the second coset $g_j H$, and so on.

In input the routine receives a logical array \texttt{sym} of size $h$ 
such that \texttt{sym(l)} is \texttt{.TRUE.} if $g_l$
belongs to $H$, and the multiplication table of
the group $G$: $t(g_i,g_j)$, where $g_i g_j = g_{t(g_i,g_j)}$.


\section{Appendix: Kpoint\_grid}

This routine generates a mesh of $N_1\times N_2\times N_3$ ${\bf k}$ points 
given $N_1$ (called
\texttt{nk1}), $N_2$ (called \texttt{nk2}) and $N_3$ (called \texttt{nk3})
as:
\begin{equation}
{\bf k}_{ijk} = \left[{(i-1) \over N_1} + {k_1\over 2 N_1}\right] {\bf b}_1 +
                \left[{(j-1) \over N_2} + {k_2\over 2 N_2}\right] {\bf b}_2 +
                \left[{(k-1) \over N_3} + {k_3\over 2 N_3}\right] {\bf b}_3 
\end{equation}
where $1\le i \le N_1$, $1\le j \le N_2$, $1\le k \le N_3$ and $k_1$,
$k_2$ and $k_3$ are $0$ or $1$.
At a generic iteration, the routine takes the first ${\bf k}_i$ in the
list not already found in previous iterations and rotates it with all 
operations of the point group $G$ of the Bravais lattice. If the rotated 
point is in the original list, it is removed and the 
weight of the point ${\bf k}_i$ is increased by 1. In output 
for each star of ${\bf k}_i$ (for $G$) we have only one point and its weight 
is the number of points of the star of ${\bf k}_i$ which were in the 
original grid.
So the total weight is $N_1 N_2 N_3$. The weight are finally normalized
so as to have sum $1$.

\section{Appendix: Irrek}
This routine receives a list of ${\bf k}$ points (\texttt{nks} points) 
$\{{\bf k}_1, {\bf k}_2,...,{\bf k}_N\}$ with their weights
$\{w_1,...,w_N\}$ that have been reduced by a point group $G$ with $h$ 
(\texttt{nrot}) elements, for instance those produced by 
the \texttt{kpoint\_grid} routine.
Given a subgroup $H$ of $G$ with $k$ (\texttt{nsym}) elements, it gives 
the ${\bf k}$ points and their weights reduced with the group $H$.

The algorithm is the following: given ${\bf k}_i$ we rotate it with 
all elements of $G$ and obtain the star of ${\bf k}_i$ (for G). 
Given ${\bf k}_i$ rotating with all elements of $H$ we obtain the star 
of ${\bf k}_i$ for $H$. Of this star we have to save only the point 
${\bf k}_i$. 
Let's consider now a generic coset $g_l H$ and let's apply all its
elements to ${\bf k}_i$. This gives the point 
$g_l {\bf k}_i$ and its star for $H$. Now if the point $g_l {\bf k}_i$ 
is not contained in all previous stars, we have to add $g_l {\bf k}_i$ 
to the list ${\bf k}$. If it is contained in the star of
$g_j {\bf k}_i$ we have to account that the coset $g_jH$ has an additional
star. The weight of each new $g_l {\bf k}_i$ will be the weight of
${\bf k}_i$ when reduced with $G$, divided by the number of cosets and
multiplied by the number of stars equivalent to $g_l {\bf k}_i$. 
...
in the noncollinear magnetic case as:
\begin{equation}
...
\ref{dynmat_inv_smallq} to symmetrize the dynamical matrix. We can rewrite Eq.~\ref{symdyn2} in the basis of the primitive reciprocal lattice vectors inserting four
$\delta$ in it:
\begin{eqnarray}
D_{s,\alpha,s',\beta}({\bf q}) &=& {1\over N_{S_{\bf q}}}
\sum_{i=1}^{N_{S_{\bf q}}}
...
\label{symdyn0_mq}
\end{eqnarray}
So if we have a unsymmetrized dynamical matrix we can use it with the equation
\begin{equation}
D_{s,\alpha,s',\beta}({\bf q})=
{1\over 2}
...
\section{Appendix: Input variables}

Input variables which might influence the symmetry. 

\begin{itemize}
\item
\texttt{nosym=.TRUE.}. When this flag is set to \texttt{.TRUE.} the code 
removes all symmetries. The ${\bf k}$ points given in input are used as 
they are. If a grid of ${\bf k}$ points is specified in input, the mesh
is generated and only $-{\bf k}$, ${\bf k}$ 
equivalence due to time reversal is exploited when time reversal is present.
To remove time reversal set $\texttt{noinv=.TRUE.}$ or do a noncollinear
calculation with a starting magnetization. 
Note that in general this is not equivalent to a self consistent calculation
in which symmetry is used when the original mesh does not transform in
itself with the point group symmetries of the Bravais lattice.

\item
\texttt{nosym\_evc=.TRUE.}. When this flag is set to \texttt{.TRUE.}
the ${\bf k}$ points in input or the ${\bf k}$ points of a grid are
used together with all the points generated by the point group symmetries
of the Bravais lattice. However the point group of the solid is assumed
to contain only identity so ${\bf k}$ points are not reduced with
the symmetry of the point group of the solid. This calculation is 
equivalent to a self-consistent calculation in which symmetry is used
and should give the same results. $-{\bf k}$, ${\bf k}$
equivalence due to time reversal is exploited when time reversal is
present. To remove time reversal set $\texttt{noinv=.TRUE.}$ or do a 
noncollinear calculation with a starting magnetization.
