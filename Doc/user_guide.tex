...
\subsection{\color{web-blue}\texttt{what='scf\_elastic\_constants\_qha'}}

With this option the code computes the elastic constants and elastic
...
elastic constants with the \texttt{elastic\_algorithm='energy'}, the code 
computes the phonon dispersions and computes the elastic constants
at each temperature as the second derivatives of the Helmholtz free energy 
...
Only one unperturbed geometry, the one given in input is used with 
this option.

\subsection{\color{web-blue}\texttt{what='elastic\_constants\_t\_qha'}}

This option is similar to \texttt{what='scf\_elastic\_constants\_qha'} but
makes the temperature dependent elastic constants calculation on a regular 
grid of unperturbed geometries, the same geometries chosen with the option
\texttt{what='mur\_lc\_t'}, and writes them on separate files, one for
...
obtained by interpolating the ``quasi-harmonic'' elastic constants computed 
by this option at the crystal parameters found at each temperature from 
the minimization of the free energy.
This option is presently limited to cubic crystals and it is 
very experimental.
