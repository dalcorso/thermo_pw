...
\item Band structure at the minimum of the total energy.
...
\item Phonon dispersions on a mesh of lattice parameters and calculation of the
anharmonic vibrational properties: lattice parameters, 
isochoric heat capacity and thermal expansion as a function of temperature
(limited to cubic, tetragonal, orthorombic, and hexagonal systems).
Bulk modulus and pressure derivative of the bulk modulus, isobaric heat 
capacity, isoentropic bulk modulus and average Gr\"uneisen parameters as 
...
output of the code are the values of celldm as a function of
...
Finally, the Gr\"uneisen parameters interpolated from quadratic fits
of the frequencies as a function of the crystal parameters
are shown on the same path used for the phonon
dispersions. The input variables that control these plots are those
described in the option \texttt{what='mur\_lc'} and 
\texttt{what='mur\_lc\_disp'}.
From celldm as a function of temperature the code computes the thermal
expansion tensor and the volume thermal expansion. \\
This option is implemented only for tetragonal, hexagonal, trigonal
and orthorombic systems.
For cubic systems, the Murnaghan equation is used to interpolate the
free energy and in addition to the lattice constant as a function of
...
For anisotropic solids derivatives of the frequencies are calculated always
with \texttt{with\_eigen=.TRUE.}, but presently they are not used to calculate 
anharmonic properties.
The input variables that control these plot are those described in the option
...
volume_ph     : The frequencies and Gruneisen parameters interpolated 
                ...
                When volume_ph=0.0_DP the volume is calculated from temp_ph.
                This option is available only for cubic solids. Otherwise
                use celldm_ph.
                Default : 0.0_DP (in (a.u.)**3)
celldm_ph     : The frequencies and Gruneisen parameters interpolated 
                ...
                If this is 0.0_DP the celldm are calculated from temp_ph.
                To have accurate Gruneisen parameters and intepolated
                frequencies set the central geometry as close as possible 
                to celldm_ph. The central geometry is the one given in the
                input of pw.x.
                Default : 0.0_DP (celldm(1) in a.u., celldm(2-6) adimensional)
temp_ph       : The frequencies and Gruneisen parameters interpolated at 
                the volume that corresponds to this temperature are plotted 
                on a postscript file (only when volume_ph=0.0_DP).
...
independent strains. \\
%As a rule the user is not supposed to change the elastic algorithm.
%The code will choose the one more suited to the given lattice. The variable
%\texttt{elastic\_algorithm} is used for tests or when the stress is 
%not available for a given functional. The latter case can be dealt with by
%the algorithm \texttt{energy} when it is available for the given lattice.
...
\subsection{\texttt{what='scf\_polarization'}}
