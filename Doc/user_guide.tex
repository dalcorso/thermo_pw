...
Then you need to obtain the source code of the \thermo\ package. 
\texttt{Thermo\_pw} can be downloaded from the 
\texttt{qe-forge} website at \texttt{http://www.qe-} 
\texttt{forge.org/gf/project/thermo\_pw/frs/}.
There are several versions. The one that you need depends on the version 
of \qe\ that you are using. 
The following table is the compatibility table between \qe\ and 
\texttt{thermo\_pw} versions. For the versions of \qe\ not listed in
the table there is no compatible \texttt{thermo\_pw} package.
\begin{center}
\begin{table}
\begin{tabular}{ll}
\hline
...
\end{table}
\end{center}
Also the CVS version of the \texttt{thermo\_pw} directory can be downloaded as 
described at
...
if you have problems to download the CVS version of \texttt{thermo\_pw}
or if the combination \texttt{thermo\_pw (CVS)} + \texttt{QE (SVN)} is not 
working.
...
with the command \texttt{tar -xzvf thermo\_pw.0.3.0.tar.gz}, you 
...
lambda_elem : The anode element, used to set the X-ray wavelenght.
...
lxrdp       : if .TRUE. compute the xrdp also after the cell optimization
              with all the options mur_lc_... with the uniformely strained
...
Young modulus, the shear modulus and the Poisson ratio for polycrystalline
...
the bulk modulus, the polycrystalline averages of the Young modulus,
...
The Voigt-Reuss-Hill average of the shear modulus and the bulk modulus are 
...
Debye vibrational energy, free energy, entropy and constant volume heat
capacity. These quantities are plotted in a postscript file as a function
of temperature which is specified as explained for \texttt{what='scf\_disp'}.
...
It is also possible to stop the \texttt{thermo\_pw} run after the calculation
of the phonon dispersions in a fixed number of geometries (input variable
\texttt{max\_geometries}).
...
\item \texttt{test\_colors.x} produces a page with the gnuplot colors that     
can be used in the plots.   
...
In addition to this user guide, this directory contains the following
documents:
