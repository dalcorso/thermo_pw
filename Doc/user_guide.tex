...
\item Frozen ions and real elastic constants.
\end{itemize}
...
equilibrium positions. Finally it computes the elastic constants from
the numerical derivatives of the stress with respect to strain.
The number of independent strains is \texttt{ngeo}. The input variables
that control this option are:
\begin{verbatim}
delta_epsilon : the interval of strain values between two geometries.
                The central geometry has zero strain (ngeo must be odd).
\end{verbatim}
Presently only the cubic case (simple cubic, fcc, and bcc) is implemented.

\subsection{\texttt{what='fi\_elastic\_constants'}}
As \texttt{what='elastic\_constants'} but the ionic positions are not 
relaxed, but only strained according to each strain.
