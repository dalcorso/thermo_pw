...
\texttt{thermo\_pw/} that should be put in the root directory of the \qe\ tree:
...
\hline
\hline
\texttt{thermo\_pw} & \qe & release date \\
\hline
1.5.0 & 6.7 &  \\
...
Please match carefully \texttt{thermo\_pw} and \qe\ versions
as illustrated in Table 1. 
For the versions of \qe\ not listed here, there is no \texttt{thermo\_pw} 
package. The source files of \texttt{thermo\_pw} can be obtained by
...
\texttt{thermo\_pw/src/thermo\_pw.x} that appears in the 
...
a file called \texttt{thermo\_control}, and an input for \texttt{ph.x}
...
fcfact      : factor used to convert from the 2\pi/a units used 
              for the Brillouin zone plot to the millimeters units needed to
...
lb0_t         : if .TRUE. the calculation of the thermal expansion with 
                ...
                are not computed.
                Default: logical .TRUE.
poly_degree_grun : degree of the polynomial used to interpolate the frequencies
