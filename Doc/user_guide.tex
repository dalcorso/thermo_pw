...
require several steps and the passage of data from one program to the other.
These tasks are usually carried out by bash scripts that require
some time to be prepared. \thermo\ allows to speed up this part of the 
work for the most common material properties. It can be considered as a set 
...
several geometries are calculated independently, and in parallel if there
are several images, for computing the total energy for a Murnaghan fit.  
...
Presently \texttt{thermo\_pw} works only with the SVN version of \qe, there
is no version that works with the current 5.0.3 version of \qe.
The \texttt{thermo\_pw} directory can be downloaded with the command 
\texttt{cvs -d :ext:anonymous@qeforge.qe-forge.org:}
\texttt{/cvsroot/thermo\_pw co thermo\_pw}. 
...
and \texttt{ph.x} before \texttt{thermo\_pw}
...
This option makes several self-consistent calculations, in parallel on several
images, varying the kinetic energy cut-off for the wavefunctions and for the 
charge density. 
...
This option makes several self-consistent calculations, in parallel on several
images, varying the size of the {\bf k}-point grid, and optionally for metals
the smearing parameter \texttt{degauss}. In the input of \texttt{pw.x} 
the minimum value of these parameters is given and these values are 
increased in fixed intervals controlled by the following variables. On output 
the energy is plotted as a function of the mesh size, one curve for each 
smearing parameter.
...
With these options the code makes a self-consistent calculation followed 
...
A phonon calculation is made at a single {\bf q} point or on a mesh of {\bf q}
points, as specified in the input of the \texttt{ph.x} code after a 
self-consistent \texttt{pw.x} calculation. No additional calculation is done 
by the \thermo\ code. The different representations are calculated in 
parallel when several images are available.
No other input variable is necessary. The output of this calculation
are the dynamical matrices files.
An example of the use of this option can be found in \texttt{example03}.

\subsection{\texttt{what='scf\_disp'}}
A phonon dispersion calculation is made after the self-consistent run at
a fixed geometry. The geometry is given in the input of \texttt{pw.x}. 
The dynamical matrices are used to calculate the interatomic force
constants and to interpolate the phonon dispersions along a path in the
Brillouin zone. The path can be generated automatically on the basis of
the Bravais lattice index or given in input as in a band structure
calculation (see above \texttt{what='scf\_bands'}). The code then interpolates
the phonon on a uniform mesh of {\bf q} points and computes the density
of vibrational states with a smearing approach. The density of states is 
used to calculate the harmonic thermodynamical properties: vibrational energy,
vibrational free energy, vibrational entropy, and isochoric heat capacity.
...
This option computes the band structure at the geometry obtained from the
minimum of the Murnaghan equation. See the options \texttt{what='scf\_bands'} 
and \texttt{what='mur\_lc'} for a list of the variables that control these
two options. They are used in the same way with the present option.
...
Using this option the \thermo\ code calculates the anharmonic vibrational
