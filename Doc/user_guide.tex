...
Using images in a phonon calculation has an overhead because each image must 
recalculate the band structure at each task, or check if the bands are 
already on disk, calculated previously by the same images. On some systems 
with slow disks it could be faster to recalculate the bands instead of 
reading them from disk. 
An input variable allows forcing the band calculation:
\begin{verbatim}
