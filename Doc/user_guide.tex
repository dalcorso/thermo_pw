...
\texttt{ibrav} and to use the primitive vectors provided by \qe.
...
find_ibrav : This variable is active only when continue_zero_ibrav=.FALSE..
             ...
             and of the atomic positions that produce the same crystal as
             given in input and continue the calculation. The geometry 
...
It can give the names of the space group given the ITA number, or
the number given one of the names, translate the names between different
editions of the ITA tables or the Sh\"oneflies names. It gives the list 
...
however, before generating the supercell, the code rotates the Bravais
lattice so that it has the same orientation of the vectors described
in the \texttt{thermo.pdf} guide.

\item \texttt{bravais\_lattices.x} is a small tool that tests the
...
\item \texttt{hex\_trig.x} is a small tool that reads the values of $a$ and $c$
...
be put directly in the input of \texttt{pw.x} for this type of cells.

\item \texttt{translate.x} reads a set of atomic positions
and translates them of a fixed vector. If a rotation matrix is given in input
it can also rototranslate the set of atomic positions.
...
matrices, the class structure of the point group and of the double point
group, the character tables of the point group, of the double point
group and the characters of the projective representations of the point
group, the list of subgroups and supergroups of a given group and
the compatibility tables of a given group with its subgroups.
