...
nppl: the number of k points per string. In the perpendicular 
      ...
      Default: integer 21
\end{verbatim}
This feature is still experimental. Pools are not available with Berry 
phase calculations.
...
Number of tasks for this option: \texttt{ngeo\_strain} times the number of
strain types. \\
This feature is still experimental. Pools are not available for Berry phase
calculations.
%
...
ltau_from_file : if .TRUE. the atomic coordinates of each geometry
             ...
             calculation.
             Default : .FALSE.
save_distorted_energy : if .TRUE. the code writes in restart directory
             the energy files for all distorted geometries that 
             do not change the lattice using the polynomial 
             that interpolates the energy.
             Default : .FALSE.
