...
\texttt{ngeo(1)}$+$\texttt{ngeo(2)}$+$\texttt{ngeo(3)}$+$\texttt{ngeo(4)}$+$\texttt{ngeo(5)}$+$\texttt{ngeo(6)}-5$ when 
...
lv0_t         : if .TRUE.  the calculation of the thermal expansion with 
                ...
                If reduced_grid=.TRUE. we must have  lv0_t=.FALSE. and
                the input geometry is used. 
                Default: logical .TRUE.
