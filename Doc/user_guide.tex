...
\title{
  \includegraphics[width=8cm]{thermo_pw_logo.png} \\
  \vspace{3truecm}
  % title
...
\item Electronic thermodynamic properties: energy, free energy, entropy, 
and heat capacity (for metals only). 
...
\item Frozen ions and relaxed ions zero temperature elastic constants 
      at fixed geometry.

\item Berry phase polarization at fixed geometry.

\item Piezoelectric tensor at zero temperature at fixed geometry.

\item Magnetic susceptibility at zero temperature at fixed geometry.

\item Magnetoelectric tensor at zero temperature at fixed geometry.
...
\item Berry phase polarization at the minimum of the total energy.

\item Piezoelectric tensor at zero temperature at the minimum of the
total energy.

\item Magnetic susceptibility at the minimum of the total energy.

\item Magnetoelectric tensor at the minimum of the total energy.

\item Frozen ions and relaxed ions zero temperature elastic constants
in a grid of geometries.
...
\item Internal thermal expansion as a function of temperature within
the ZSISA approximation or within the full free energy minimization (FFEM)
(only for selected crystal structures).

\item Frozen ions and relaxed ions temperature dependent elastic constants
in a grid of geometries within the quasi-harmonic approximation.

\item Frozen ions and relaxed ions piezoelectric tensor at zero temperature
in a grid of geometries.

\item Berry phase polarization at zero temperature in a grid of geometries.
...
\item Piezoelectric tensor in a grid of geometries and interpolation at the 
geometry that minimizes the free energy at a given temperature and pressure
...
These programs are distributed in the hope that they will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHAN-TABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details.
...
\hline
2.2.1 & 7.6   &            \\
2.2.0 & 7.5   & 13/08/2026 \\
2.1.1 & 7.5   & 13/10/2025 \\
2.1.0 & 7.4.1 & 24/09/2025 \\
2.0.3 & 7.4.1 & 23/06/2025 \\
2.0.2 & 7.4 & 20/06/2025 \\
2.0.1 & 7.3.1 & 15/04/2025 \\
2.0.0 & 7.3 & 24/09/2024 \\
...
With some versions of QE the sequence of commands which is
known to compile \texttt{thermo\_pw} is the following:
\begin{verbatim}
Untar qe (with the version you want to run)
tar -xzvf qe.tgz
cd main qe directory
tar -xzvf thermo\_pw.tzg
cd thermo_pw
make join_qe
cd ..
./configure
make thermo_pw
\end{verbatim}
Running \texttt{./configure} before \texttt{make join\_qe} might not 
work properly. To run \texttt{thermo\_pw} with \texttt{GPU} you need
a \texttt{pgi} fortran compiler (for instance the one contained in the 
\texttt{Nvidia hpc sdk}) and should run \texttt{configure} with
a command similar to:
\begin{verbatim}
./configure --with-cuda=$CUDA_ROOT --with-cuda-runtime=11.8 
--with-cuda-cc=80 --with-scalapack=no --with-cuda-mpi=yes
\end{verbatim}
Please check on your system which is the correct version of 
\texttt{cuda-runtime} and of \texttt{cuda-cc}. The numbers given here
are indicative.
...
This option is not yet available for \texttt{lsda} calculations.
...
njump         : the geometries for the generalized Gruneisen method are 
                chosen each njump geometries of the grid in each direction.  
                Default: Integer, 1
...
If you know in which direction the atoms relax and if there is only one
atom that moves in each strain type, one can use \texttt{frozen\_ions=.TRUE.},
and compute the energy at several atomic positions choosing the one
that minimizes the energy. This approach computes more energy configurations 
but does not relax them and should provide results identical to 
\texttt{frozen\_ions=.FALSE.}. It can be used only with \texttt{energy}
and \texttt{energy\_std} algorithms, but should work also with the
free energy in \texttt{what=`elastic\_constants\_geo'}.
...
nmove        Number of atomic positions sampled for minimizing 
             ...
             Default: integer 5.
stypec(1:strain_types): For each strain type for which this 
             variable is .TRUE. it is possible to compute 
             the energy by moving atoms along a manifold in the 
             coordinates space and minimize the energy with 
             respect to the parameters that determine the manifold. 
             This option requires frozen_ions=.TRUE. and should 
             give the same result of frozen_ions=.FALSE. if the 
             atom that moves and the manifold are chosen correctly.
             Default: logical .FALSE..
iconstr_internal_ec(1:strain_types): The constraint number that 
             determines which manifold is used to describe atomic 
             motion.
             Default: integer none (it must be set 
             if stypec(istrain)=.TRUE.).
nint_var_ec(1:strain_types): The dimension of the manifold for 
             each strain type.
             Default: integer 1 
int_ngeo_ec(1:max_nint_var,1:strain_types): the number of displacements
             for each dimension of the manifold.
             Default : integer 5 
int_step_ngeo_ec(1:max_nint_var,1:strain_types): the step of the 
             displacement for each dimension of the manifold.
             Default: real 0.005
...
\newpage
{\color{coral}\section{what='scf\_polarization'}}
\color{black}
...
\texttt{frozen\_ions=.TRUE.} require also a previous \texttt{what='scf'}
calculation where the atomic positions of the equilibrium configuration
are calculated and written on file. \texttt{ltau\_el\_cons\_from\_file=.TRUE.}
must be used.
...
\newpage
{\color{coral}\section{what='scf\_piezoelectric\_tensor'}}
\color{black}
With this option the code calculates the stress piezoelectric tensor 
($e_{\alpha,m}$ $\alpha=1,3,\ m=1,6$) of the solid. Both the proper
and improper tensors are calculated.
Depending on the point group, it calculates the polarization of the
solid for a set of strains. For each strain, it relaxes 
the ions to their equilibrium positions when \texttt{frozen\_ions=.FALSE.} 
or keeps them in the strained positions when \texttt{frozen\_ions=.TRUE.}. 
Finally it computes the piezoelectric tensor from the numerical derivatives 
of the polarization (improper piezoelectric tensor) or of the Berry phase
(proper piezoelectric tensor) with respect to strain.
For each strain type, the number of strains used to compute each derivative 
is \texttt{ngeo\_strain}.\\
If the flag \texttt{decompose\_piezo} is set to .TRUE. and the output
of a previous \texttt{frozen\_ions=.TRUE.} calculation is on file, this
option prints also the decomposition of the piezoelectric tensor into a
clamped-ion and ionic constributions. This calculation requires also
the Born effective charges written inside a dynamical matrix at the
$\Gamma$ point of the equilibrium configuration.
If the clamped-ion term is not found on file only the ionic contribution
is printed and the clamped ion term is deduced from the difference with
the relaxed piezoelectric tensor. If the Born effective charges are
not found on file only the derivative of the internal parameters with
respect to strain are calculated. \texttt{decompose\_piezo} works
only for the components of the piezoelectric tensor derived for the strain
types for which the user specifies the constraint number.

The \texttt{what='scf\_piezoelectric\_tensor'} option supports the
\texttt{standard} (default) and 
\texttt{advanced} elastic algorithms with the same meaning as explained
for the option \texttt{what='scf\_elastic\_constants}.
...
the strain piezoelectric tensor $d_{\alpha,m}$ which describes
the polarization induced by an external stress, as $d_{\alpha,m}=
\sum_n e_{\alpha,n} C_{nm}^{-1}$. All these quantities are calculated 
...
{\color{coral}\section{what='scf\_magnetic\_susceptibility'}}
\color{black}
With this option the code calculates the magnetic susceptibility 
($\chi^M_{\alpha,\beta}$ $\alpha=1,3,\beta=1,3$) of the solid. 
It generates a mesh of \texttt{ngeo\_b} values of the magnetic field 
for each direction required by symmetry and computes the magnetization. 

For each magnetic field, it relaxes 
the ions to their equilibrium positions when \texttt{frozen\_ions=.FALSE.} 
or keeps them in the original positions when \texttt{frozen\_ions=.TRUE.}. 
Finally it fits the 
magnetization with a linear or cubic polynomial and extracts the 
...
dm:          A vector of size dm is added to the starting magnetization
             ...
             Default : real 0.002
\end{verbatim}
Number of tasks for this option: \texttt{ngeo\_b} times the number of
directions that need to be studied. 
%
\newpage

{\color{coral}\section{what='scf\_magnetoelectric\_tensor'}}
\color{black}
With this option the code calculates the magnetoelectric tensor
($\alpha_{\alpha,\beta}$ $\alpha=1,3,\beta=1,3$) of the solid. 
It generates a mesh of \texttt{ngeo\_b} values of the magnetic field 
for each direction required by the magnetic point group and after 
converging it computes the Berry phase polarization of the system. 
Finally it makes the derivatives of the polarization with respect 
...
Therefore the resulting magnetoelectric tensor contains both the electronic
and ionic contribution in the first case, or only the electronic contribution
in the second case. When using \texttt{frozen\_ions=.TRUE.} it is important
to calculate the equilibrium atomic positions and not use those strained
from the \texttt{thermo\_pw} input using the flag
\texttt{ltau\_el\_cons\_from\_file} (please check the use of this 
flag in the \texttt{what='scf\_elastic\_constants'} option.

To control the grid of magnetic fields used please see the 
\texttt{what='scf\_magnetic\_susceptibility'} option.
...
option.
It is also possible to optimize the atomic positions by doing, for each
set of crystal parameters, a set of self consistent calculations at several
atomic positions. These atomic positions must be specified introducing
internal parameters (a vector of dimension \texttt{nint\_var}). The
code generates a uniform mesh of internal parameters and the user
must support a routine that transforms the internal parameters in
atomic coordinates or the atomic coordinates
into internal parameters. This routine is called \texttt{internal\_to\_tau}.
...
do_scf_relax : if .TRUE. the code makes a self-consistent relax 
             ...
             Default: logical .FALSE. 
linternal_thermo: if .TRUE. we do the calculation optimizing internal
             parameters. 
             Default : logical .FALSE.
iconstr_internal: an integer code that chooses the constraint 
             implemented in the routine internal_to_tau.
             See the thermo.pdf manual for the list of contraints 
             implemented so far.
             Default: integer 1
nint_var:    the dimension of the internal variables space. Presently 2 is
             the maximum value supported.
             Default: integer 1
int_ngeo(nint_var) : the number of points in the mesh of internal variables
             generated by the code.
             Default: integer 5
int_step_ngeo(nint_var) : the distance between points in the internal
             variable mesh. This variable has the same units of the
             parameter that it represents. See the thermo.pdf manual
             for a description.
             Default: real(DP) 0.005
linterpolate_tau : if .TRUE. the atomic positions of each geometry
             read from the dynamical matrices are
             used to find the internal parameters and the latter are
             interpolated at the temperature (and pressure) dependent
             crystal parameters. The plots shows the temperature (and
             pressure) dependence of these internal parameters (within the
             ZSISA approximation). This flag requires a iconstr_internal
             value. To see the FFEM internal parameter use linternal_thermo
             flag.
             Default: .FALSE.
...
lgeo_to_file : if .TRUE. at the end of the calculation the code
             ...
             Default : .FALSE.
ltau_from_file : if .TRUE. the atomic coordinates of each geometry
             are read from file. They must be in the restart
             directory calculated in a previous what='mur_lc' 
             calculation.
             Default : .FALSE.
save_distorted_energies : if .TRUE. the code writes in energy_dist 
             directory the energy files for all distorted geometries 
             that do not change the lattice type using the polynomial 
             that interpolates the energy. The distorted
             geometries are the same as those generated by
...
\newpage
{\color{coral}\section{what='mur\_lc\_polarization'}}
\color{black}
...
\newpage
{\color{coral}\section{what='mur\_lc\_piezoelectric\_tensor'}}
\color{black}
As \texttt{what='scf\_piezoelectric\_tensor'} but the calculation 
is made at the
geometry that minimizes the energy. The
energy minimization is made as described for \texttt{what='mur\_lc'}. \\

{\color{coral}\section{what='mur\_lc\_magnetic\_susceptibility'}}
\color{black}
As \texttt{what='scf\_magnetic\_susceptibility'} but the calculation 
is made at the
geometry that minimizes the energy. The
energy minimization is made as described for \texttt{what='mur\_lc'}. \\

{\color{coral}\section{what='mur\_lc\_magnetoelectric\_tensor'}}
\color{black}
As \texttt{what='scf\_magnetoelectric\_tensor'} but the calculation
is made at the
geometry that minimizes the energy. The
energy minimization is made as described for \texttt{what='mur\_lc'}. \\
...
lv0_t         : if .TRUE.  the calculation of the thermal 
                ...
                the input geometry is used when lv0_t=.FALSE.
                Default: logical .TRUE.
...
lhugoniot     : If .TRUE. the code plots T(p) and V(p) along the 
                Hugoniot curve.
                Default: .FALSE.
save_distorted_energies : if .TRUE. the code writes in energy_dist directory
                the thermodynamic functions for all distorted geometries that 
                do not change the lattice type using a polynomial 
                to interpolate the thermodynamic functions. The distorted 
                geometries are the same as those generated by 
                what='elastic_constants_geo'.
                Default : .FALSE.
...
The unperturbed geometries can also be chosen by the user using the
variable \texttt{lgeo\_from\_file=.TRUE.}.
...
\newpage
{\color{coral}\section{what='polarization\_geo'}}
\color{black}

With this option the code computes the Berry phase polarization in a grid 
of geometries, the geometries specified as in \texttt{what='mur\_lc'}.
The results are written in different files in the directory 
\texttt{elastic\_constants}. With a subsequent calculation that
use \texttt{what='mur\_lc'} or \texttt{what='mur\_lc\_t'} the code
can read the file produced with this option and interpolate the
polarization at the pressure (and temperature if \texttt{what='mur\_lc\_t'})
dependent geometries using polynomials. 
The calculation of polarization is controled by the same variables used
for the option \texttt{what='scf\_polarization'}.
The variables that control this option are:
%\begin{tcolorbox}
\begin{verbatim}
fl_polar : the name of the file that contains the polarization.
         Default : 'output_polar.dat'
\end{verbatim}


\newpage
{\color{coral}\section{what='piezoelectric\_tensor\_geo'}}
\color{black}

With this option the code computes the piezoelectric tensor in a grid 
of geometries, the geometries specified as in \texttt{what='mur\_lc'}.
The results are written in different files in the directory 
\texttt{elastic\_constants}. With a subsequent calculation that
use \texttt{what='mur\_lc'} or \texttt{what='mur\_lc\_t'} the code
can read the file produced with this option and interpolate the
piezoelectric tensor at the pressure (and temperature if 
\texttt{what='mur\_lc\_t'}) dependent geometries. 
Usually the piezoelectric coefficient is calculated using
\texttt{frozen\_ions=.FALSE.} relaxing the atomic positions for 
each distorted configuration. In this case the piezoelectric tensor 
is computed within ZSISA.
It is also possible to use the flag \texttt{use\_free\_energy=.TRUE.}. 
Together with the flags \texttt{stypec} 
(see the option \texttt{what='scf\_elastic\_constants'}) 
the code can compute the internal parameters as a function of strain
by minimizing the free energy at each temperature. These temperature 
dependent internal 
parameters are used to compute the temperature dependent piezoelectric
tensors within the full free energy minimization (FFEM).
...
\newpage
{\color{coral}\section{\texttt{what='mur\_lc'}, \texttt{what='mur\_lc\_t'} and elastic constants}}
\color{black}

In order to plot the elastic constants as a function of temperature 
within the `quasi-harmonic' approximation,
it is necessary to make a calculation with \texttt{what='mur\_lc\_t'} 
having on files in the directory \texttt{anhar\_files} the elastic 
constants calculated for each geometry 
with the option \texttt{what='elastic\_cons-} \texttt{ants\_geo'}. 
...
constants computed by the option \texttt{what='elastic\_constants\_geo'} 
at the crystal parameters found at each temperature from the minimization 
of the free energy. 

Note that the user must be careful to use the same value for the
\texttt{lel\_free\_} \texttt{energy} flag both in the calculation of the 
``fixed-geometry quasi-harmonic'' elastic
constants and in the present \texttt{mur\_lc\_t} calculation that 
interpolates the elastic constants.

A run of \texttt{thermo\_pw} using 
\texttt{what='mur\_lc\_t'} having on files the $T=0$ K elastic constants
in the directory \texttt{elastic\_constants}
allows the calculation of the anharmonic properties using temperature
dependent elastic constants and bulk moduli obtained by interpolating 
(within the ``quasi-static approximation") the $T=0$ K elastic constants 
at the crystal parameters that, at each temperature, 
minimize the free energy. 
When both the $T=0$ K and the temperature dependent elastic constants
are on file in the directory \texttt{elastic\_constants} and 
\texttt{anhar\_files} respectively, the latter are used.

After computing the elastic constants at $T=0$ K with the option
\texttt{what='ela-} \texttt{stic\_constants\_geo'}
...
plotted on output. A plot of the pressure dependent stress-strain elastic
...
on NVIDIA GPUs. It has been tested on \texttt{marconi100} 
at CINECA loading modules \texttt{hpc-sdk/2022--binary} or 
\texttt{hpc-sdk/2023--binary} and on \texttt{Leonardo} at CINECA loading
modules \texttt{nvhpc/23.1} and \texttt{openmpi/4.1.4--nvhpc--23.1-cuda}
 \texttt{-11.8}.
...
\item \texttt{analyze\_dynmat.x} reads two dynamical matrices files. The 
first is supposed to be at the $\Gamma$ point, the other at a small 
${\bf q}$. It applies the acoustic sum rule, the nonanalytic term
if needed at $\Gamma$, and diagonalizes both matrices. 
...
\item \texttt{wurtzite.x} Some tools to work with the wurtzite structure.
Given a, c/a, and u writes the coordinates of the four atoms or given
a, c/a, and the coordinates of the atoms finds u.
...
\item
\texttt{symmetry.pdf}: some notes explaining how symmetry is used 
to reduce calculations in electronic structure codes.
