...
but presently (23/01/2016) the \texttt{SVN} version 
of \texttt{QE} can be downloaded only by \texttt{QE} developers.
...
with the command \texttt{tar -xzvf thermo\_pw.0.4.0.tar.gz}, you 
...
\subsection{\texttt{what='scf\_dos'}}
With this option the code makes a self-consistent calculation followed
...
so that the present code is not suited to obtain accurate thermodynamic 
...
described for \texttt{what='scf\_bands'} are obtained for all the 
...
files that contains the elastic constants in the working directory. 
...
the dynamical matrices files, the \texttt{save\_energy} file and the
\texttt{restart} directory and rerun \thermo.
