...
give the stress acting on the solid and this gives one or more equations
...
$\zeta_2=\cos\alpha$. $a$ is the modulus of the three primitive vectors 
and $\alpha$ their angle.

It is also useful to associate to the rhombohedral lattice an hexagonal 
lattice whose unit cell contains three points of the rhombohedral lattice.
Indicating with $a_h$ and $c_h$ the parameters of the hexagonal lattice we
have:
\begin{eqnarray}
...
From the hexagonal lattice parameters we can derive the size of
the rhombohedral lattice:
\begin{eqnarray}
a&=& {1\over 3} \sqrt{3 a_h^2 + c_h^2} \\
\cos\alpha&=&{2 c_h^2-3 a_h^2 \over 2 ( 3 a_h^2+c_h^2)}
\end{eqnarray}
or equivalently:
\begin{equation}
\cos\alpha={ (c_h/a_h)^2-3/2 \over  3 +(c_h/a_h)^2}
\end{equation}
...
Applying $\epsilon_A$, $\epsilon_B$ or $\epsilon_E$ to a rhombohedral 
lattice does not change its shape. 

Applying $\epsilon_A$ changes $a_r$ to $a_r (1+\epsilon_{1,1})$ while the
rhombohedral angle does not change.

Applying $\epsilon_B$ changes both $a_r$ and the rhombohedral angle 
$\alpha_r$. We have 
\begin{eqnarray}
a&=&a_t \sqrt{(1 + \epsilon_{1,1})^2 \sin^2\theta_r + \cos^2\theta_r}, \\
\cos\alpha &=& {
2 \cos^2\theta_r -(1 + \epsilon_{1,1})^2 \sin^2\theta_r  \over
 \left[(1 + \epsilon_{1,1})^2 \sin^2\theta_r+ 2 \cos^2\theta_r\right]}
\end{eqnarray}
where 
...
In order to derive these equations it is useful to note that given the
strained vector ${\bf a}'_1$, ${\bf a}'_2$ and ${\bf a}'_3$ the basis
vectors of a monoclinic lattice can be taken as ${\bf a}_{1M}= {\bf a}'_1+
{\bf a}'_3$, ${\bf a}_{2M}= {\bf a}'_1- {\bf a}'_3$ and
${\bf a}_{3M}={\bf a}'_2$. In the rotated reference system $y'$ is along
$x$ (parallel to ${\bf a}_{2M}$), $x'$ is parallel to ${\bf a}_{1M}$, 
while $z'$ is orthogonal to $x'$ and $y'$.
...
The reference system of the base-centered monoclinic lattice is rotated 
with respect to the reference system of the rhombohedral lattice, as 
described before, but this time the rotation angle depends on strain.
The change of coordinates can be written as above with the rotation matrix 
\begin{equation}
...
Applying $\epsilon_{CI}=\epsilon_C+\epsilon_I$ to a rhombohedral lattice 
transforms it into a base centered monoclinic lattice with:
\begin{eqnarray}
a&=&a_r \sqrt{(1+\epsilon_{2,3}^2)(1 + 3 \cos^2 \theta_r)-
8\epsilon_{2,3}\sin \theta_r\cos \theta_r}, \\
b&=&a_r \sqrt{3}(1+\epsilon_{1,1}) \sin \theta_r, \\
...
8\epsilon_{2,3}\sin \theta_r\cos \theta_r} \sqrt{1+\epsilon_{2,3}^2+4\epsilon_{2,3}\sin \theta_r\cos \theta_r}}.
\end{eqnarray}
The rotation matrix remains the same of the case $\epsilon_I$.

Applying $\epsilon_{B'}$ to a rhombohedral lattice transforms it into a
base-centered $b$-unique monoclinic lattice with parameters:
\begin{eqnarray}
a&=&a_r \sqrt{ \sin^2 \theta_r + 4 (1+\epsilon_{1,1})^2 \cos^2 \theta_r} \\
b&=&a_r \sqrt{3}(1+\epsilon_{1,1})\sin \theta_r, \\
c&=&a_r \sqrt{\sin^2 \theta_r + (1+\epsilon_{1,1})^2 \cos^2 \theta_r}\\
\cos\beta &=& { -\sin^2 \theta_r + 2 (1+\epsilon_{1,1})^2 \cos^2 \theta_r
\over 
\sqrt{\sin^2 \theta_r + (1+\epsilon_{1,1})^2 \cos^2 \theta_r}
\sqrt{\sin^2 \theta_r + 4 (1+\epsilon_{1,1})^2 \cos^2 \theta_r}}
\end{eqnarray}

The rotation matrix has the same form seen for $\epsilon_I$ but with
\begin{eqnarray}
A&=& {- \sin \theta_r \over 
\sqrt{\sin^2 \theta_r + 4 (1+\epsilon_{1,1})^2 \cos^2 \theta_r}}, \\
B&=& {2 (1+\epsilon_{1,1}) \cos \theta_r \over 
\sqrt{\sin^2 \theta_r + 4 (1+\epsilon_{1,1})^2 \cos^2 \theta_r}}, \\
\end{eqnarray}
...
\appendix
{\color{dark-blue}\chapter{Constrains}}
\color{black}

This appendix contains a list of the constraints already implemented in
\texttt{thermo\_pw} that can be addressed with the variable
\texttt{iconstr\_internal}.

{\color{web-blue}\subsection{\texttt{iconstr\_internal=1}}}
\color{black}
This constraint is suited for the wurtzite structure. This structure has
an hexagonal lattice with two external parameters $a$ and $c/a$ and 
four atoms in the unit cell. This constraint assumes that the atoms have
the following positions (taking $ZnO$ as example):
\begin{eqnarray}
\boldsymbol{\tau}_{1,Zn} &=& ( 0, 0, 0), \nonumber \\
\boldsymbol{\tau}_{2,Zn} &=& ( 1/2, -\sqrt{3}/6, c/2a), \nonumber \\
\boldsymbol{\tau}_{3,O} &=& ( 0, 0, u c/2a), \nonumber \\
\boldsymbol{\tau}_{4,O} &=& ( 1/2, -\sqrt{3}/6, (1/2 + u)c/a). \nonumber 
\end{eqnarray}
There is one internal adimensional degree of freedom $u$ whose value is 
close to the ideal $u=3/8$. The atomic coordinates are in units of $a$.

{\color{web-blue}\subsection{\texttt{iconstr\_internal=2}}}
\color{black}
This constraint is suited to a distorted hcp structure. The unperturbed 
hcp structure has an hexagonal lattice with two external parameters
$a_h$ and $c_h/a_h$ and two atoms in the unit cell:
\begin{eqnarray}
\boldsymbol{\tau}_{1,Zn} &=& ( 0, 0, 0), \nonumber \\
\boldsymbol{\tau}_{2,Zn} &=& ( 1/2, \sqrt{3}/6, c_h/2a_h). \nonumber 
\end{eqnarray}
A strain $(\epsilon,0,0,0,0,0)$ transforms the hexagonal lattice to a 
base centered orthorombic lattice with $a=(1+\epsilon) a_h$,
$b = \sqrt{3} a_h$, $c = c_h$.
The atomic positions (always in units of $a_h$) become:
\begin{eqnarray}
\boldsymbol{\tau}_{1,Zn} &=& ( 0, -y \over 2, 0), \nonumber \\
\boldsymbol{\tau}_{2,Zn} &=& ( (1+\epsilon)/2, y_0+{y\over 2}, c_h/2 a_h). \nonumber 
\end{eqnarray}
where $y$ is a adimensional parameter and $y_0= \sqrt{3}/6$. 
This routine mush give as output the difference between the uniformly 
strained coordinates and the actual coordinates in a.u..
\begin{eqnarray}
\Delta \boldsymbol{\tau}_{1,Zn} &=& a_h ( 0, -y / 2  , 0), \nonumber \\
\Delta \boldsymbol{\tau}_{2,Zn} &=& a_h ( 0, {y\over 2}, 0). \nonumber 
\end{eqnarray}
$a_h y$ is the internal coordinate which is given as input and is in a.u..

{\color{web-blue}\subsection{\texttt{iconstr\_internal=3}}}
\color{black}
This constraint is suited to a distorted wurtzite structure. The unperturbed 
wurtzite structure has an hexagonal lattice with two external parameters
$a_h$ and $c_h/a_h$ and four atoms in the unit cell:
\begin{eqnarray}
\boldsymbol{\tau}_{1,Zn} &=& ( 0, 0, 0), \nonumber \\
\boldsymbol{\tau}_{2,Zn} &=& ( 1/2, -\sqrt{3}/6, c_h/2a_h), \nonumber \\
\boldsymbol{\tau}_{3,O} &=& ( 0, 0, u_0 c_h/2a_h), \nonumber \\
\boldsymbol{\tau}_{4,O} &=& ( 1/2, -\sqrt{3}/6, (1/2 + u_0)c_h/a_h). \nonumber 
\end{eqnarray}
where $u_0$ is an internal parameter that characterize the wurtzite
structure.
A strain $(\epsilon,0,0,0,0,0)$ transforms the hexagonal lattice to a 
base centered orthorombic lattice with $a=(1+\epsilon) a_h$,
$b = \sqrt{3} a_h$, $c = c_h$.
...
\begin{eqnarray}
\boldsymbol{\tau}_{1,Zn} &=& ( 0, -{y\over 2}, 0), \nonumber \\
\boldsymbol{\tau}_{2,Zn} &=& ( (1+\epsilon)/2, y_0+{y\over 2}, c_h/2a_h), \nonumber \\
\boldsymbol{\tau}_{3,O} &=& ( 0, -{y\over 2}, (u_0+u_1) c_h/2a_h), \nonumber \\
\boldsymbol{\tau}_{4,O} &=& ( (1+\epsilon)/2, y_0+{y\over 2}, (1/2 + (u_0+u_1))c_h/a_h). \nonumber 
\end{eqnarray}
where $y$ is an internal adimensional parameter and $y_0= \sqrt{3}/6$. 
This routine mush give as output the difference between the uniformly 
strained coordinates and the actual coordinates in a.u..
\begin{eqnarray}
\Delta \boldsymbol{\tau}_{1,Zn} &=& a_h ( 0, -{y\over 2}, 0), \nonumber \\
\Delta \boldsymbol{\tau}_{2,Zn} &=& a_h ( 0, {y\over 2}, 0), \nonumber \\
\Delta \boldsymbol{\tau}_{3,O} &=& a_h ( 0, -{y\over 2}, u_1 {c_h\over 2 a_h}), \nonumber \\
\Delta \boldsymbol{\tau}_{4,O} &=& a_h ( 0, {y\over 2}, u_1 {c_h \over 2 a_h}). \nonumber 
\end{eqnarray}
$a_h y$ and $u_1$ are the internal coordinates which form a two dimensional
space and are given as input. The first is in a.u., the second is adimensional.
