...
\newpage
\section{\color{coral}Stress and pressure dependent elastic constants}
...
the energy with respect to the strains:
\begin{equation}
{1 \over V}{\partial^2 U \over \partial \epsilon_{ij} \partial \epsilon_{kl}}
\Bigg|_{\epsilon=0},
\end{equation}
do not coincide neither with the elastic constant
...
relationship between $B_{ij,kl}$ and the calculated
...
To find the elastic constants of a solid under pressure we can specialize
this formula to the case in which $\sigma^{(0)}_{ij}=-p \delta_{ij}$. We
have:
...
We can write out the relationship for the Voigt components:
\begin{eqnarray}
B_{44} &=& {1 \over V}{\partial^2 U \over \partial \epsilon^2_{4}}\Bigg|_{\epsilon=0} - {1\over 2} p, \\
B_{55} &=& {1 \over V}{\partial^2 U \over \partial \epsilon^2_{5} 
}\Bigg|_{\epsilon=0} - {1\over 2} p, \\
B_{66} &=& {1 \over V}{\partial^2 U \over \partial \epsilon^2_{6} 
}\Bigg|_{\epsilon=0} - {1\over 2} p, \\
B_{12} &=& {1 \over V}{\partial^2 U \over \partial \epsilon_{1} 
\partial \epsilon_{2}}\Bigg|_{\epsilon=0} + p, \\
B_{13} &=& {1 \over V}{\partial^2 U \over \partial \epsilon_{1} 
\partial \epsilon_{3}}\Bigg|_{\epsilon=0} + p, \\
B_{23} &=& {1 \over V}{\partial^2 U \over \partial \epsilon_{2} 
\partial \epsilon_{3}}\Bigg|_{\epsilon=0} + p, 
\end{eqnarray}
while for all the other components $B_{ij}=
{1 \over V}{\partial^2 U \over \partial \epsilon_{i} 
\partial \epsilon_{j}}\big|_{\epsilon=0}$.

For a generic stress, as opposed to an isotropic pressure, the $B_{ij,kl}$ 
have not the same symmetry properties of $C_{ij,kl}$. In general they 
cannot be written in the Voigt notation.
...
%minimizes the free energy at temperature $T$. 
%We have:
%\begin{equation}
%C_{i,l}^T= {1 \over V(T)}{\partial^2 F \over \partial \epsilon_i \partial \epsilon_l}
%\Bigg|_T.
%\end{equation}
%One approximation consists in computing the elastic constants as a function 
%of temperature taking as unstrained geometry the equilibrium one
%at $T=0$. The disadvantage of this approach is that at each temperature
%the solid has a stress.
%
