...
%  \includegraphics[width=6cm]{\SissaImage}\\
  \vskip 1cm
  \Huge Thermo\_pw: Thermodynamics of crystals \\
...
vectors depend on a few parameters $\alpha_k$ that determine the size 
...
characterized by a set of parameters $\alpha_{k,0}$ and a set of parameters 
$\alpha_k$ corresponds to a solid in a strained state.
...
stresses and using these quantities we can obtain the adiabatic heat
capacity, elastic constants, and bulk moduli from the corresponding 
isothermal ones. Finally from the constant strain heat capacity and 
the thermal stresses we can calculate the anisotropic average Gr\"uneisen 
parameters.
...
parameters $\alpha_k$ to the thermodynamic functions.
...
All three lattices are characterized by one parameter $\alpha_1=a$.

\subsection{Tetragonal system}
...
Both lattices are characterized by two parameters $\alpha_1=a$ and 
$\alpha_2=c/a$.

\subsection{Orthorhombic system}
...
$\alpha_1=a$, $\alpha_2=b/a$ and $\alpha_3=c/a$.

\subsection{Monoclinic system}
...
All lattices are characterized by four parameters $\alpha_1=a$, $\alpha_2=b/a$,
$\alpha_3=c/a$ and $\alpha_4=\cos\gamma$
($c$-unique) or $\cos\beta$ ($b$-unique).

\subsection{Triclinic system}
...
$.
The lattice is characterized by six parameters $\alpha_1=a$, 
$\alpha_2=b$, $\alpha_3=c$, $\alpha_4=\cos\alpha$,
$\alpha_5=\cos\beta$, $\alpha_6=\cos\gamma$.

\subsection{Hexagonal system}
...
The lattice is characterized by two parameters $\alpha_1=a$ 
and $\alpha_2=c/a$.

\subsection{Trigonal system}
...
\cos\theta$. \\
The lattice is characterized by two parameters $\alpha_1=a$ and 
$\alpha_2=\cos\alpha$.

\section{The strain tensor}
...
by changing the parameters $\alpha_k$ that define the unit cell and 
...
%\subsection{Triclinic and Monoclinic system}
...
Also the stress is a symmetric tensor so it is possible to use the Voigt
...
the stress is $\sigma$ then $W=-V \sum_i \sigma_i d\epsilon_i$, where $V$
...
\section{Equilibrium geometry and thermal expansion}
...
$\alpha_k$ and to check the minimum condition:
\begin{equation}
{\partial U \over \partial \alpha_k}=0.
\end{equation}
Similarly at temperature $T$ the equations that define the lattice 
parameters are:
\begin{equation}
{\partial F \over \partial \alpha_k}=0.
\end{equation}
...
we obtain the parameters $\alpha_k (T)$ that can be compared with
...
$\alpha_k (T)$ we can calculate the volume $V(T)$
...
$\alpha_{k}(T)$. The thermal expansion tensor $\alpha_{i}$ is defined 
as the strain $d\epsilon_{i}(T)$ induced by a change of temperature $dT$. 
We can write
\begin{equation}
\alpha_{i} = {d \epsilon_{i}(T) \over  d T }.
\end{equation}
We have only the crystal parameters $\alpha_k (T)$ and their derivatives
...
induced by a temperature must conserve the shape of the unit cell, so
${d \alpha_k (T) \over d T}$ suffice to calculate the physically
interesting strains. We discuss this point separately for each crystal
system.

\subsection{Cubic system}
In the cubic system $k=1$ and $\alpha_1=a$. By symmetry the strain that can
...
\subsection{Tetragonal and hexagonal systems}
In the tetragonal and hexagonal systems $k=2$, $\alpha_1=a$, $\alpha_2=c/a$.
...
\subsection{Trigonal system}
In the trigonal system $k=2$, $\alpha_1=a$, $\alpha_2=\cos\alpha$. The 
...
\subsection{Orthorhombic systems}
In the orthorhombic system $k=3$, $\alpha_1=a$, $\alpha_2=b/a$, 
$\alpha_3=c/a$. The nonzero components of the thermal expansion are 
...
%\subsection{Monoclinic systems}
%In the monoclinic system $k=4$, $\alpha_1=a$, $\alpha_2=b$, $\alpha_3=c$, 
%$\alpha_4=\cos\gamma$ (c-unique) of $\alpha_4=\cos\beta$ (b-unique). 
...
The heat capacity measure the variation of the temperature of the solid
...
\section{Pressure dependent elastic constants}
...
using the stress-strain relation it is common to give the numerical
values of $B_{ij,kl}$ and to call these the elastic constants of a
...
Note however that for a generic stress, instead of
...
%\section{Temperature dependent elastic constants}
...
%a space whose dimension are the parameters $\alpha_k$ that define the 
...
%simply changing the parameters $\alpha_k$, but in some cases it is an
...
%\end{equation}
%
%\section{Pressure and temperature dependent elastic constants}
%
...
\section{The elastic compliances}
...
\section{Bulk modulus and compressibility}
...
%respect to the crystal parameters $\alpha_k$.
%This depends on the Laue class and on the Bravais lattice, so we discuss
%this in the following.

\section{Thermal expansion from Gr\"uneisen parameters}
...
\over d c/a}
\end{equation}
\section{Average Gr\"uneisen parameter}
