...
%
%In order to compare $abA$ and $A$ we can write $abA= {1\over K_A}\ A$ 
%and reasoning
%as before we have:
%\begin{equation}
%{\tilde \mu_0 \over 2 \pi}\ {10^5\over K_A^2} = 1
%\end{equation}
%or
%\begin{equation}
%K_A = \sqrt {10^5 \tilde \mu_0 \over 4 \pi} ={1\over \tilde c} 
%\sqrt {10^5 \over 4 \pi \tilde \epsilon_0}={K\over \tilde c_{cgs}} 
%=\kappaa.
%\end{equation}
%\\
%In the old SI units ${\tilde \mu_0 \over 4 \pi} = 10^{-7}$, so $K_A=10^{-1}$.
%}
%\\
...
%$\bar I=\bari\ K_A abA=\baricgsm\ abA$.
}
\\
...
%$abA/cm^2 = {1 \over K_A 10^{-4}} A/m^2=\curtocurm\ A/m^2$.
%\\
%The continuity equation can be derived by considering
%a charge density $\rho\ statC/cm^3= {1\over 10^{-6} K}\ \rho\ C/m^3$ and
%a current density ${\bf J}\ abA/cm^2= {1 \over K_A 10^{-4}} {\bf J}\ A/m^2.$
%Accounting also for the different length units and 
%using Eq.~\ref{continuitysi} we find:
%\begin{equation}
%{1\over 10^{-6} K} {\partial \rho \over \partial t}=-{1\over 10^{-6} K_A} {\rm div} {\bf J},
%\label{continuitycgsm0}
%\end{equation}
%or in modified c.g.s.-Gaussian units:
%\begin{equation}
%{1\over c} {\partial \rho \over \partial t}=-{\rm div} {\bf J}.
...
%With the same reasoning as before and using $A=K_A\ abA$ we find:
%\begin{equation}
%{\tilde \mu_0\over 2 \pi} K_T G= {2 \over 10^2} K_A statC/cm^2
%\end{equation}
%that gives again
%\begin{equation}
% K_T = {10^3 \over K_A}=\toverg.
%\end{equation}
%\\
%}
...
%\begin{equation}
%\bar \mu_{cgs} = {10^{-4}\over \tilde c_{cgs} K_A}\ A\cdot m^2 
%={10^{-4}\over K}\ A\cdot m^2 
%= \mutomu\ A\cdot m^2
%\end{equation}
...
%\begin{equation}
%\bar H_{cgs}= {1\over 4 \pi\ K_A\ 10^{-2}} A/m=\htoh\ A/m
%\end{equation}
%\\
...
In the SI the macroscopic Maxwell's equations can be formally
...
\section{\color{coral} Appendix C: Conversion factors tables}
