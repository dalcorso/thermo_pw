...
and \thermo\ will make the conversion for you before running the job. 
...
\newpage
{\color{coral}\section{How do I calculate temperature-dependent elastic 
constants while relaxing internal degrees of freedom?}}

This calculation is performed within the ZSISA approximation
when \texttt{what='scf\_elastic\_constants'}, 
\texttt{what='mur\_lc\_elastic\_constants'}, or
\texttt{what='elastic\_constants\_geo'}.
For a given set of crystal parameters, ions are relaxed to their 
$0$ K equilibrium positions (using \texttt{frozen\_ions=.FALSE.}), 
...
freedom (currently limited to a maximum of $3$), you can compute the 
...
To move a single atom in one direction: Use \texttt{stype} and related 
input variables.
To utilize internal degrees of freedom: Use \texttt{stypec} and related
input variables. 
...
{\color{coral}\section{How do I calculate the piezoelectric tensor?}}
\color{black}
To compute the piezoelectric tensor, use the 
\texttt{what ='scf\_piezo-} \texttt{electric\_tensor'} option 
(see example25). This option
calls \texttt{pw.x} to perform Berry phase polarization calculations 
...
\texttt{what='mur\_lc\_piezoelectric\_tensor'}.
Additionally, you can 
compute the clamped-ion version of the tensor by setting 
\texttt{clamped\_ions=.TRUE.}. 
...
Perform a preliminary run using \texttt{what='scf'} or 
\texttt{what='mur\_lc'}.
This step generates the necessary atomic position files in the
\texttt{restart} directory.
Once the relaxation run is complete, you must manually move the data:
Copy the generated atomic position files into the \texttt{restart} 
...
\texttt{ltau\_el\_cons\_from\_file=.TRUE.}.

\newpage

{\color{coral}\section{How do I calculate the pyroelectric tensor?}}
\color{black}
To compute the pyroelectric tensor, use the \texttt{what = 'mur\_lc\_t'} 
option. This calculation is triggered when the code identifies three 
files in the \texttt{anhar\_files} directory: the temperature-dependent 
...
{\color{coral}\section{How do I calculate the internal coordinates as
a function of temperature?}}
\color{black}

To compute internal coordinates versus temperature, set 
\texttt{what='mur} \texttt{\_lc\_t'} and activate the interpolation flag 
by setting \texttt{linterpolate\_tau =.TRUE.}.
Before running the calculation, you must identify the internal 
coordinates for your specific solid. This requires a custom routine 
that maps atomic positions to internal coordinate vectors (and vice versa).
The code currently includes pre-programmed routines for wurtzite and 
trigonally distorted zincblende structures. You must modify the 
\texttt{internal\_to\_tau} routine located in the file 
\texttt{set\_tau\_acc.f90}.
Assign a unique integer to \texttt{iconstr\_internal} 
(ensure it is not already in use) and set this value in the 
\texttt{thermo\_control} namelist. If your internal vector is 
multidimensional, define its size using the \texttt{nint\_var} variable.
The output depends on which thermodynamic approximation you wish to use:
ZSISA is default, FFEM is obtained with \texttt{linternal\_thermo = .TRUE.}.

\newpage

{\color{coral}\section{How do I calculate the piezoelectric tensor as 
a function of temperature?}}
\color{black}
This calculation is performed within the quasi-static approximation. 
To execute it, use \texttt{what = 'mur\_lc\_t'}. 
The code requires the piezoelectric tensors for each geometry generated 
by the \texttt{what='mur\_lc\_t'} run. You have two options for 
providing these files: Place the files in the \texttt{elastic\_constants/} 
directory. If the tensors were generated using FFEM, you may instead copy 
them directly into the \texttt{anhar\_files} directory.
Once the calculation is complete, the code will generate files containing 
the piezoelectric tensor values interpolated for each temperature 
(and pressure) point in the \texttt{anhar\_files} directory and plots them.
...
for each geometry?}}
\color{black}
To compute the piezoelectric tensor across various geometries, set 
\texttt{what='piezoelectric\_tensor\_geo'}.
...
\texttt{what='mur\_lc'} as a baseline to generate a set of 
...
The resulting files are saved in the \texttt{elastic\_constants/} directory.
...
Use \texttt{stypec}, \texttt{iconstr\_internal\_ec}, and 
\texttt{nint\_var\_ec} (refer to the User’s Manual for specific 
mapping values) and set the flag \texttt{use\_free\_energy=.TRUE.}.
...
\texttt{anhar\_files} directory and are ready to be interpolated 
across your temperature (and/or pressure) range.

\newpage
{\color{coral}\section{How do I calculate temperature-dependent 
elastic constants at constant electric displacement?}}
\color{black}

To perform this calculation, use the option \texttt{what='mur\_lc\_t'}. 
The elastic constants are calculated provided the following 
data files are available for enough geometries to allow interpolation:
The elastic constants at a constant electric field ({\bf E});
The piezoelectric tensor; The dielectric properties.
The code looks for the elastic constants and piezoelectric tensor 
in the \texttt{anhar\_files} directory (if computed via QHA or FFEM) 
or the \texttt{elastic\_constants} directory (if computed only at $0$ K). 
If data exists in both locations, the code prioritizes the files 
in \texttt{anhar\_files}.
