...
constants for all the geometries used by \texttt{what='mur\_lc\_t'} and
saves them in the directory \texttt{elastic\_constants}.  
In the second step use the same input for \texttt{pw.x} and run again 
\texttt{thermo\_pw} using \texttt{what='mur\_lc\_t'} and copying in your
working directory the directory \texttt{elastic\_constants} obtained 
in the previous step. Elastic constants are read only with 
\texttt{lmurn=.FALSE.}.
...
minimizes the Helmholtz (or Gibbs at finite pressure) free energy. \\
The second method uses the ``quasi-harmonic'' approximation and
requires many phonon calculations at many geometries.
First use 
\texttt{what='elastic\_constants\_t\_qha'}. This option computes the 
...
second derivatives of the Helmholtz free energy, at the geometry that
...
second method is still experimental and limited to cubic solids.
