...
the flag \texttt{lel\_free\_energy=.TRUE.} to write on files the
...
\texttt{therm\_files} directory and run \tpw\ with \texttt{what='mur\_lc\_t'}
...
the flag \texttt{lel\_free\_energy=.TRUE.} as in the previous point
in order to get the electronic thermodynamic properties for all the 
unperturbed geometries. Then use \texttt{what='elastic\_constant\_t'}
...
and \texttt{use\_free\_energy=.TRUE.} after copying the elastic
