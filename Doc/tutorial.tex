...
{\color{coral}\section{How do I compute the piezoelectric tensor?}}
\color{black}
Use \texttt{what='scf\_piezoelectric\_tensor'}.
With this option \thermo\ calls 
\texttt{pw.x} and the berry phase calculation of the polarization for
a set of distorted structures, taking as equilibrium configuration
the geometry given in the \texttt{pw.x} input.
Finally it calculates the derivatives of the polarization with
respect to strain and finds the piezoelectric tensor components.
Using \texttt{what='mur\_lc\_piezoelectric\_tensor'} the code 
calculates the equilibrium geometry that minimizes the energy
and takes it as the equilibrium configuration.
