...
\texttt{continue\_zero\_ibrav=.TRUE.} instead \thermo\ will actually use your
...
except when you are dealing with supercells. After setting the correct
...
be useful if you have the space group and the nonequivalents atomic positions.
...
correspondence colors-representation see the \texttt{point\_groups.pdf} 
manual. The point group used for each {\bf k} or {\bf q} point is
written in the \thermo\ output. In the same output you can also find,
