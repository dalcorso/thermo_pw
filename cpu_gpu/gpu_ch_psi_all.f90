...
!
!-----------------------------------------------------------------------
SUBROUTINE ch_psi_all_tpw (n, h, ah, e, ik, m)
  !-----------------------------------------------------------------------
  !
  ...
  COMPLEX(DP), ALLOCATABLE :: h_d(:,:), hpsi_d (:,:), spsi_d (:,:)
#if defined(__CUDA)
  ATTRIBUTES(DEVICE) :: hpsi_d, spsi_d, h_d
#endif
  ! scalar products
...
#if defined(__CUDA)
  ALLOCATE (hpsi_d( npwx*npol , m))
  ALLOCATE (spsi_d( npwx*npol , m))
  ALLOCATE (h_d( npwx*npol , m))
  hpsi_d=(0.0_DP,0.0_DP)
  spsi_d=(0.0_DP,0.0_DP)
#endif
  hpsi (:,:) = (0.d0, 0.d0)
  spsi (:,:) = (0.d0, 0.d0)
  !
  ...
  IF (use_gpu) THEN
     h_d=h
     CALL h_psi_gpu (npwx, n, m, h_d, hpsi_d)
     hpsi=hpsi_d
     CALL s_psi_gpu (npwx, n, m, h_d, spsi_d)
     spsi=spsi_d
  ELSE
