...
!  =====================================================================
#if defined(__CUDA)
      ATTRIBUTES(DEVICE) SUBROUTINE ZSTEIN_XG( N, D, E, M, W, IBLOCK, & 
                         ISPLIT, Z, LDZ, WORK,IWORK, IFAIL, INFO )
#else
      SUBROUTINE ZSTEIN_XG( N, D, E, M, W, IBLOCK, ISPLIT, Z, LDZ, WORK, &
                         IWORK, IFAIL, INFO )
#endif

#include<dlagtf_interf.f90>
#include<dlagts_interf.f90>
#include<dnrm2_interf.f90>
#include<idamax_interf.f90>
#include<dlamch_interf.f90>
#include<dcopy_interf.f90>
#include<dlarnv_interf.f90>
...
!
            CALL DLARNV_XG( 2, ISEED, BLKSIZ, WORK( INDRV1+1 ) )
!
!           Copy the matrix T so it won't be destroyed in factorization.
!
            CALL DCOPY_XG( BLKSIZ, D( B1 ), 1, WORK( INDRV4+1 ), 1 )
            CALL DCOPY_XG( BLKSIZ-1, E( B1 ), 1, WORK( INDRV2+2 ), 1 )
            CALL DCOPY_XG( BLKSIZ-1, E( B1 ), 1, WORK( INDRV3+1 ), 1 )
!
!           Compute LU factors with partial pivoting  ( PT = LU )
!
            TOL = ZERO
      CALL DLAGTF_XG( BLKSIZ, WORK( INDRV4+1 ), XJ, WORK( INDRV2+2 ),   & 
...
!
            JMAX = IDAMAX_XG( BLKSIZ, WORK( INDRV1+1 ), 1 )
      SCL = BLKSIZ*ONENRM*MAX( EPS,   & 
        ABS( WORK( INDRV4+BLKSIZ ) ) ) /   & 
                  ABS( WORK( INDRV1+JMAX ) )
            CALL DSCAL_XG( BLKSIZ, SCL, WORK( INDRV1+1 ), 1 )
!
!           Solve the system LU = Pb.
!
      CALL DLAGTS_XG( -1, BLKSIZ, WORK( INDRV4+1 ), WORK( INDRV2+2 ),   & 
...
!
  130       CONTINUE
            SCL = ONE / DNRM2_XG( BLKSIZ, WORK( INDRV1+1 ), 1 )
            JMAX = IDAMAX_XG( BLKSIZ, WORK( INDRV1+1 ), 1 )
