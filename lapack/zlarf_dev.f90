...
#include<ilazlc_interf.f90>
#include<ilazlr_interf.f90>
#include<zgerc_interf.f90>
...
!      EXTERNAL           ZGEMV_XG, ZGERC_XG
!     ..
!     .. External Functions ..
!      LOGICAL            LSAME_XG
!      INTEGER            ILAZLR_XG, ILAZLC_XG
!      EXTERNAL           LSAME_XG, ILAZLR_XG, ILAZLC_XG
!     ..
...
!     Scan for the last non-zero column in C(1:lastv,:).
            LASTC = ILAZLC_XG(LASTV, N, C, LDC)
         ELSE
!     Scan for the last non-zero row in C(:,1:lastv).
            LASTC = ILAZLR_XG(M, LASTV, C, LDC)
         END IF
      END IF
...
!      CALL ZGEMV( 'Conjugate transpose', LASTV, LASTC, ONE,   & 
      CALL ZGEMV_XG( 'C', LASTV, LASTC, ONE,   & 
                 C, LDC, V, INCV, ZERO, WORK, 1 )
!
!           C(1:lastv,1:lastc) := C(...) - v(1:lastv,1) * w(1:lastc,1)**H
!
            CALL ZGERC_XG( LASTV, LASTC, -TAU, V, INCV, WORK, 1, C, LDC )
         END IF
      ELSE
!
!        Form  C * H
...
!
!      CALL ZGEMV_XG( 'No transpose', LASTC, LASTV, ONE, C, LDC,   & 
      CALL ZGEMV_XG( 'N', LASTC, LASTV, ONE, C, LDC,   & 
                 V, INCV, ZERO, WORK, 1 )
!
!           C(1:lastc,1:lastv) := C(...) - w(1:lastc,1) * v(1:lastv,1)**H
!
            CALL ZGERC_XG( LASTC, LASTV, -TAU, WORK, 1, V, INCV, C, LDC )
         END IF
      END IF
      RETURN
!
