...
#include<zungql_interf.f90>
#include<zungqr_interf.f90>
...
!      EXTERNAL           XERBLA_XG, ZUNGQL_XG, ZUNGQR_XG
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC          MAX
!     ..
...
!
      IF( INFO.EQ.0 ) THEN
         IF( UPPER ) THEN
!            NB = ILAENV_XG( 1, 'ZUNGQL', ' ', N-1, N-1, N-1, -1 )
            NB = ILAENV_XG( 1, 'E', ' ', N-1, N-1, N-1, -1 )
         ELSE
!            NB = ILAENV_XG( 1, 'ZUNGQR', ' ', N-1, N-1, N-1, -1 )
            NB = ILAENV_XG( 1, 'F', ' ', N-1, N-1, N-1, -1 )
         END IF
         LWKOPT = MAX( 1, N-1 )*NB
         WORK( 1 ) = LWKOPT
      END IF
...
!
         CALL ZUNGQL_XG( N-1, N-1, N-1, A, LDA, TAU, WORK, LWORK, IINFO )
...
!           Generate Q(2:n,2:n)
!
      CALL ZUNGQR_XG( N-1, N-1, N-1, A( 2, 2 ), LDA, TAU, WORK,   & 
                         LWORK, IINFO )
         END IF
      END IF
      WORK( 1 ) = LWKOPT
