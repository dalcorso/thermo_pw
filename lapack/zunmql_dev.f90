...
!  =====================================================================
#if defined(__CUDA)
      ATTRIBUTES(DEVICE) SUBROUTINE ZUNMQL_XG( SIDE, TRANS, M, N, K, A,LDA, &
...
!
      ...
      INFO = 0
      LEFT = LSAME_XG( SIDE, 'L' )
      NOTRAN = LSAME_XG( TRANS, 'N' )
      LQUERY = ( LWORK.EQ.-1 )
!
...
!
      ...
      ELSE
         ...
         NW = MAX( 1, M )
      END IF
      IF( .NOT.LEFT .AND. .NOT.LSAME_XG( SIDE, 'R' ) ) THEN
         INFO = -1
      ELSE IF( .NOT.NOTRAN .AND. .NOT.LSAME_XG( TRANS, 'C' ) ) THEN
         INFO = -2
      ELSE IF( M.LT.0 ) THEN
         INFO = -3
      ELSE IF( N.LT.0 ) THEN
...
!                                     K, -1 ) )
      NB = MIN( NBMAX, ILAENV_XG( 1, 'H', SIDE, M, N,   & 
                                     K, -1 ) )
            LWKOPT = NW*NB + TSIZE
         END IF
         WORK( 1 ) = LWKOPT
...
!                    -1 ) )
      NBMIN = MAX( 2, ILAENV_XG( 2, 'H', SIDE, M, N, K,   & 
                    -1 ) )
         END IF
      END IF
!
...
!        Use unblocked code
!
      CALL ZUNM2L_XG( SIDE, TRANS, M, N, K, A, LDA, TAU, C, LDC, WORK,   & 
                      IINFO )
      ELSE
!
!        Use blocked code
...
!
!      CALL ZLARFT_XG( 'Backward', 'Columnwise', NQ-K+I+IB-1, IB,   & 
      CALL ZLARFT_XG( 'B', 'C', NQ-K+I+IB-1, IB,   & 
...
!           Apply H or H**H
!
!      CALL ZLARFB_XG( SIDE, TRANS, 'Backward', 'Columnwise', MI, NI,   & 
      CALL ZLARFB_XG( SIDE, TRANS, 'B', 'C', MI, NI,   & 
