...
!      EXTERNAL           XERBLA_XG, ZLARFB_XG, ZLARFT_XG, ZUNG2L_XG
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
!     ..
...
!
      IF( INFO.EQ.0 ) THEN
         IF( N.EQ.0 ) THEN
            LWKOPT = 1
         ELSE
!            NB = ILAENV_XG( 1, 'ZUNGQL', ' ', M, N, K, -1 )
            NB = ILAENV_XG( 1, 'E', ' ', M, N, K, -1 )
            LWKOPT = N*NB
         END IF
         WORK( 1 ) = LWKOPT
!
...
!
      IF( INFO.NE.0 ) THEN
         ! CALL XERBLA_XG( 'ZUNGQL', -INFO )
         CALL XERBLA_XG( 'Y', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
!
!         NX = MAX( 0, ILAENV_XG( 3, 'ZUNGQL', ' ', M, N, K, -1 ) )
         NX = MAX( 0, ILAENV_XG( 3, 'E', ' ', M, N, K, -1 ) )
         IF( NX.LT.K ) THEN
!
...
!
               NB = LWORK / LDWORK
!               NBMIN = MAX( 2, ILAENV_XG( 2, 'ZUNGQL', ' ', M, N, K, -1 ) )
               NBMIN = MAX( 2, ILAENV_XG( 2, 'E', ' ', M, N, K, -1 ) )
            END IF
         END IF
      END IF
!
...
!
      CALL ZUNG2L_XG( M-KK, N-KK, K-KK, A, LDA, TAU, WORK, IINFO )
!
      IF( KK.GT.0 ) THEN
!
!        Use blocked code
...
!
!      CALL ZLARFT_XG( 'Backward', 'Columnwise', M-K+I+IB-1, IB,   & 
      CALL ZLARFT_XG( 'B', 'C', M-K+I+IB-1, IB,   & 
                            A( 1, N-K+I ), LDA, TAU( I ), WORK, LDWORK )
!
!              Apply H to A(1:m-k+i+ib-1,1:n-k+i-1) from the left
!

!      CALL ZLARFB_XG( 'Left', 'No transpose', 'Backward',   & 
!        'Columnwise', M-K+I+IB-1, N-K+I-1, IB,   & 
      CALL ZLARFB_XG( 'L', 'N', 'B',   & 
...
!
      CALL ZUNG2L_XG( M-K+I+IB-1, IB, IB, A( 1, N-K+I ), LDA,   & 
                         TAU( I ), WORK, IINFO )
!
