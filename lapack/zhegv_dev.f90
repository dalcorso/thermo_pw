...
#include<zheev_interf.f90>
#include<zhegst_interf.f90>
#include<zhetrd_interf.f90>
#include<zpotrf_interf.f90>
...
!      EXTERNAL           XERBLA_XG, ZHEEV_XG, ZHEGST_XG, ZPOTRF_XG, ZTRMM_XG,&
!                         ZTRSM_XG
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC          MAX
...
!
      ...
      IF( ITYPE.LT.1 .OR. ITYPE.GT.3 ) THEN
         INFO = -1
      ELSE IF( .NOT.( WANTZ .OR. LSAME_XG( JOBZ, 'N' ) ) ) THEN
         INFO = -2
      ELSE IF( .NOT.( UPPER .OR. LSAME_XG( UPLO, 'L' ) ) ) THEN
         INFO = -3
      ELSE IF( N.LT.0 ) THEN
         INFO = -4
...
!
      IF( INFO.NE.0 ) THEN
         ! CALL XERBLA_XG( 'ZHEGV ', -INFO )
         CALL XERBLA_XG( 'F', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
!
      CALL ZPOTRF_XG( UPLO, N, B, LDB, INFO )
      IF( INFO.NE.0 ) THEN
         INFO = N + INFO
         RETURN
      END IF
...
!
      CALL ZHEGST_XG( ITYPE, UPLO, N, A, LDA, B, LDB, INFO )
      CALL ZHEEV_XG( JOBZ, UPLO, N, A, LDA, W, WORK, LWORK, RWORK, INFO )
!
      IF( WANTZ ) THEN
!
...
!
            ...
            END IF
!
!      CALL ZTRSM_XG( 'Left', UPLO, TRANS, 'Non-unit', N, NEIG, ONE,   & 
      CALL ZTRSM_XG( 'L', UPLO, TRANS, 'N', N, NEIG, ONE,   & 
                        B, LDB, A, LDA )
!
...
!
            ...
            END IF
!
!      CALL ZTRMM_XG( 'Left', UPLO, TRANS, 'Non-unit', N, NEIG, ONE,   & 
      CALL ZTRMM_XG( 'L', UPLO, TRANS, 'N', N, NEIG, ONE,   & 
                        B, LDB, A, LDA )
         END IF
      END IF
!
