...
!      EXTERNAL           XERBLA_XG, ZLARFB_XG, ZLARFT_XG, ZUNG2R_XG
!     ..
!     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
!     ..
...
!
      INFO = 0
!      NB = ILAENV_XG( 1, 'ZUNGQR', ' ', M, N, K, -1 )
      NB = ILAENV_XG( 1, 'F', ' ', M, N, K, -1 )
      LWKOPT = MAX( 1, N )*NB
      WORK( 1 ) = LWKOPT
      LQUERY = ( LWORK.EQ.-1 )
      IF( M.LT.0 ) THEN
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         ! CALL XERBLA_XG( 'ZUNGQR', -INFO )
         CALL XERBLA_XG( '1', -INFO )
         RETURN
      ELSE IF( LQUERY ) THEN
         RETURN
      END IF
...
!
!         NX = MAX( 0, ILAENV_XG( 3, 'ZUNGQR', ' ', M, N, K, -1 ) )
         NX = MAX( 0, ILAENV_XG( 3, 'F', ' ', M, N, K, -1 ) )
         IF( NX.LT.K ) THEN
!
...
!
               NB = LWORK / LDWORK
!               NBMIN = MAX( 2, ILAENV_XG( 2, 'ZUNGQR', ' ', M, N, K, -1 ) )
               NBMIN = MAX( 2, ILAENV_XG( 2, 'F', ' ', M, N, K, -1 ) )
            END IF
         END IF
      END IF
!
...
!
      IF( KK.LT.N )   & 
        CALL ZUNG2R_XG( M-KK, N-KK, K-KK, A( KK+1, KK+1 ), LDA,   & 
                      TAU( KK+1 ), WORK, IINFO )
!
      IF( KK.GT.0 ) THEN
!
...
!
!      CALL ZLARFT_XG( 'Forward', 'Columnwise', M-I+1, IB,   & 
      CALL ZLARFT_XG( 'F', 'C', M-I+1, IB,   & 
                            A( I, I ), LDA, TAU( I ), WORK, LDWORK )
!
!              Apply H to A(i:m,i+ib:n) from the left
!
!      CALL ZLARFB_XG( 'Left', 'No transpose', 'Forward',   & 
!        'Columnwise', M-I+1, N-I-IB+1, IB,   & 
      CALL ZLARFB_XG( 'L', 'N', 'F',   & 
        'C', M-I+1, N-I-IB+1, IB,   & 
...
!
      CALL ZUNG2R_XG( M-I+1, IB, IB, A( I, I ), LDA, TAU( I ), WORK,   & 
                         IINFO )
!
