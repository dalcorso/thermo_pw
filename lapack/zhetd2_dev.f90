...
!      EXTERNAL           XERBLA_XG, ZAXPY_XG, ZHEMV_XG, ZHER2_XG,
!                         ZLARFG_XG
!     ..
!     .. External Functions ..
!      LOGICAL            LSAME_XG
...
!
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         ! CALL XERBLA_XG( 'ZHETD2', -INFO )
         CALL XERBLA_XG( 'L', -INFO )
         RETURN
      END IF
!
...
!
            ALPHA = A( I, I+1 )
            CALL ZLARFG_XG( I, ALPHA, A( 1, I+1 ), 1, TAUI )
            E( I ) = DBLE( ALPHA )
!
...
!
      ...
      CALL ZHEMV_XG( UPLO, I, TAUI, A, LDA, A( 1, I+1 ), 1, ZERO,   & 
                           TAU, 1 )
!
!              Compute  w := x - 1/2 * tau * (x**H * v) * v
!
               ALPHA = -HALF*TAUI*ZDOTC_XG( I, TAU, 1, A( 1, I+1 ), 1 )
               CALL ZAXPY_XG( I, ALPHA, A( 1, I+1 ), 1, TAU, 1 )
!
!              Apply the transformation as a rank-2 update:
!                 A := A - v * w**H - w * v**H
!
      CALL ZHER2_XG( UPLO, I, -ONE, A( 1, I+1 ), 1, TAU, 1, A,   & 
                           LDA )
!
            ELSE
               A( I, I ) = DBLE( A( I, I ) )
...
!
            ALPHA = A( I+1, I )
            CALL ZLARFG_XG( N-I, ALPHA, A( MIN( I+2, N ), I ), 1, TAUI )
            E( I ) = DBLE( ALPHA )
!
            IF( TAUI.NE.ZERO ) THEN
!
...
!
      CALL ZHEMV_XG( UPLO, N-I, TAUI, A( I+1, I+1 ), LDA,   & 
                           A( I+1, I ), 1, ZERO, TAU( I ), 1 )
!
!              Compute  w := x - 1/2 * tau * (x**H * v) * v
!
      ALPHA = -HALF*TAUI*ZDOTC_XG( N-I, TAU( I ), 1, A( I+1, I ),   & 
                       1 )
               CALL ZAXPY_XG( N-I, ALPHA, A( I+1, I ), 1, TAU( I ), 1 )
!
!              Apply the transformation as a rank-2 update:
!                 A := A - v * w**H - w * v**H
!
      CALL ZHER2_XG( UPLO, N-I, -ONE, A( I+1, I ), 1, TAU( I ), 1,   & 
                           A( I+1, I+1 ), LDA )
!
            ELSE
