...
!
      ...
      INFO = 0
      IF( .NOT.( LSAME_XG( SIDE, 'L' ) .OR. LSAME_XG( SIDE, 'R' ) ) ) THEN
         INFO = 1
      ELSE IF( .NOT.( LSAME_XG( PIVOT, 'V' ) .OR. LSAME_XG( PIVOT,   & 
               'T' ) .OR. LSAME_XG( PIVOT, 'B' ) ) ) THEN
         INFO = 2
      ELSE IF( .NOT.( LSAME_XG( DIRECT, 'F' ) .OR. LSAME_XG( DIRECT, 'B' ) ) )   & 
                THEN
         INFO = 3
      ELSE IF( M.LT.0 ) THEN
         INFO = 4
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         ! CALL XERBLA_XG( 'ZLASR ', INFO )
         CALL XERBLA_XG( 'O', INFO )
         RETURN
      END IF
!
...
!
   ...
   10                CONTINUE
                  END IF
   20          CONTINUE
            ELSE IF( LSAME_XG( DIRECT, 'B' ) ) THEN
               DO 40 J = M - 1, 1, -1
                  CTEMP = C( J )
                  STEMP = S( J )
                  IF( ( CTEMP.NE.ONE ) .OR. ( STEMP.NE.ZERO ) ) THEN
