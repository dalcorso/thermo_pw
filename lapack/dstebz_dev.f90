...
!     Decode RANGE
!
      IF( LSAME_XG( RANGE, 'A' ) ) THEN
         IRANGE = 1
      ELSE IF( LSAME_XG( RANGE, 'V' ) ) THEN
         IRANGE = 2
      ELSE IF( LSAME_XG( RANGE, 'I' ) ) THEN
         IRANGE = 3
      ELSE
         IRANGE = 0
      END IF
...
!     Decode ORDER
!
      IF( LSAME_XG( ORDER, 'B' ) ) THEN
         IORDER = 2
      ELSE IF( LSAME_XG( ORDER, 'E' ) ) THEN
         IORDER = 1
      ELSE
         IORDER = 0
      END IF
...
!
      ...
      ULP = DLAMCH_XG( 'P' )
      RTOLI = ULP*RELFAC
!      NB = ILAENV_XG( 1, 'DSTEBZ', ' ', N, -1, -1, -1 )
      NB = ILAENV_XG( 1, 'I', ' ', N, -1, -1, -1 )
      IF( NB.LE.1 )   & 
         NB = 0
!
...
!
         ...
         IWORK( 6 ) = IU
!
      CALL DLAEBZ_XG( 3, ITMAX, N, 2, 2, NB, ATOLI, RTOLI, PIVMIN, D, E,   & 
...
!
            WORK( N+1 ) = GL
            WORK( N+IN+1 ) = GU
      CALL DLAEBZ_XG( 1, 0, IN, IN, 1, NB, ATOLI, RTOLI, PIVMIN,   & 
...
!
      ITMAX = INT( ( LOG( GU-GL+PIVMIN )-LOG( PIVMIN ) ) /   & 
                    LOG( TWO ) ) + 2
      CALL DLAEBZ_XG( 2, ITMAX, IN, IN, 1, NB, ATOLI, RTOLI, PIVMIN,   & 
        D( IBEGIN ), E( IBEGIN ), WORK( IBEGIN ),   & 
