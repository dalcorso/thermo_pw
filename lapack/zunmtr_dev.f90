...
#include<zunmql_interf.f90>
#include<zunmqr_interf.f90>
...
!
      ...
      ELSE IF( .NOT.UPPER .AND. .NOT.LSAME_XG( UPLO, 'L' ) ) THEN
         INFO = -2
      ELSE IF( .NOT.LSAME_XG( TRANS, 'N' ) .AND. .NOT.LSAME_XG( TRANS, 'C' ) )   & 
                THEN
         INFO = -3
      ELSE IF( M.LT.0 ) THEN
         INFO = -4
...
!                    -1 )
      NB = ILAENV_XG( 1, 'H', SIDE, M-1, N, M-1,   & 
                    -1 )
            ELSE
!      NB = ILAENV( 1, 'ZUNMQL', SIDE // TRANS, M, N-1, N-1,   & 
!                    -1 )
      NB = ILAENV_XG( 1, 'H', SIDE, M, N-1, N-1,   & 
                    -1 )
            END IF
         ELSE
            IF( LEFT ) THEN
...
!                    -1 )
      NB = ILAENV_XG( 1, 'G', SIDE, M-1, N, M-1,   & 
                    -1 )
            ELSE
!      NB = ILAENV( 1, 'ZUNMQR', SIDE // TRANS, M, N-1, N-1,   & 
!                    -1 )
      NB = ILAENV_XG( 1, 'G', SIDE, M, N-1, N-1,   & 
                    -1 )
            END IF
         END IF
         LWKOPT = NW*NB
...
!
      CALL ZUNMQL_XG( SIDE, TRANS, MI, NI, NQ-1, A( 1, 2 ), LDA, TAU, C,   & 
...
!
         ...
         ELSE
            ...
            I2 = 2
         END IF
      CALL ZUNMQR_XG( SIDE, TRANS, MI, NI, NQ-1, A( 2, 1 ), LDA, TAU,   & 
