...
!
         ...
         IF( LSAME_XG( UPLO, 'U' ) ) THEN
            DO 20 J = 1, N
               DO 10 I = 1, J - 1
                  SUM = ABS( A( I, J ) )
                  IF( VALUE .LT. SUM .OR. DISNAN_XG( SUM ) ) VALUE = SUM
   10          CONTINUE
               SUM = ABS( DBLE( A( J, J ) ) )
               IF( VALUE .LT. SUM .OR. DISNAN_XG( SUM ) ) VALUE = SUM
   20       CONTINUE
   ...
   40       CONTINUE
         END IF
      ELSE IF( ( LSAME_XG( NORM, 'I' ) ) .OR. ( LSAME_XG( NORM, 'O' ) ) .OR.   & 
               ( NORM.EQ.'1' ) ) THEN
!
...
!
  ...
  100       CONTINUE
         END IF
      ELSE IF( ( LSAME_XG( NORM, 'F' ) ) .OR. ( LSAME_XG( NORM, 'E' ) ) ) THEN
!
!        Find normF(A).
!
         SCALE = ZERO
         ...
         IF( LSAME_XG( UPLO, 'U' ) ) THEN
            DO 110 J = 2, N
               CALL ZLASSQ_XG( J-1, A( 1, J ), 1, SCALE, SUM )
  110       CONTINUE
         ELSE
            DO 120 J = 1, N - 1
               CALL ZLASSQ_XG( N-J, A( J+1, J ), 1, SCALE, SUM )
  120       CONTINUE
         END IF
         SUM = 2*SUM
         DO 130 I = 1, N
