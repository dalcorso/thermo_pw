...
MODULE bp
!---------------------------------------------------------------------------------
  !! Contains the variables needed for the Berry phase polarization calculation
  !
  USE kinds,   ONLY: DP
  ...
  PUBLIC :: lberry, lelfield, lorbm, gdir, nppstr, nberrycyc, evcel, evcelp, evcelm, &
            fact_hepsi, bec_evcel, mapgp_global, mapgm_global, nppstr_3d, ion_pol,   &
            el_pol, fc_pol, l_el_pol_old, el_pol_old, el_pol_acc, nx_el, l3dstring,  &
            efield, efield_cart, efield_cry, transform_el, mapg_owner, phase_control
  PUBLIC :: allocate_bp_efield, deallocate_bp_efield, bp_global_map
  PUBLIC :: pdl_tot
  !
  !
  LOGICAL :: lberry = .FALSE.
  !! if .TRUE. calculate polarization using Berry phase
  LOGICAL :: lelfield = .FALSE.
  !! if .TRUE. finite electric field using Berry phase
  LOGICAL :: lorbm = .FALSE.
  !! if .TRUE. calculate orbital magnetization (Kubo terms)
  INTEGER :: gdir
  !! G-vector for polarization calculation
  INTEGER :: nppstr
  !! number of k-points (parallel vector)
  INTEGER :: nberrycyc
  !! number of cycles for convergence in electric field 
  !! without changing the selfconsistent charge
  REAL(DP) :: efield
  !! electric field intensity in a.u.
  ...
  REAL(DP) :: fc_pol(3)
  !! the prefactor for the electronic polarization
  LOGICAL  :: l_el_pol_old
  !! if true there is already stored a n older value for the polarization
  !! neeeded for having correct polarization during MD
  REAL(DP) :: el_pol_old(3)
  !! the old  electronic polarization
  REAL(DP) :: el_pol_acc(3)
  !! accumulator for the electronic polarization
  !
  INTEGER :: nppstr_3d(3)
  !! number of element of strings along the reciprocal directions
  INTEGER, ALLOCATABLE :: nx_el(:,:)
  !! index for string to k-point map, (nks*nspin,dir=3)
  LOGICAL :: l3dstring
  !! if true strings are on the 3 three directions
  REAL(DP) :: efield_cart(3)
  !! electric field vector in cartesian units
  REAL(DP) :: efield_cry(3)
  !! electric field vector in crystal units
  REAL(DP) :: transform_el(3,3)
  !! transformation matrix from cartesian coordinates to normed reciprocal
  !! space
  INTEGER, ALLOCATABLE :: mapg_owner(:,:)
  REAL(DP) :: pdl_tot
  !! the total phase calculated from bp_c_phase
  INTEGER  :: phase_control
  !! 0 no control, 1 write, 2 read
  !
 CONTAINS
  !
  !------------------------------------------------------------------------- 
 ...
 SUBROUTINE bp_global_map
   ...
   ENDDO
   !
   CALL mp_sum( ln_g(:,:,:), intra_bgrp_comm )
   !
   ...
   ENDDO
   !
   CALL mp_sum( g_ln(:,:), intra_bgrp_comm ) 
   !
   !loop on direction
    DO idir = 1, 3
    ...
    mapg_owner = 0
    DO ig = 1, ngm
       mapg_owner(1,ig_l2g(ig)) = me_bgrp + 1
       mapg_owner(2,ig_l2g(ig)) = ig
    ENDDO
    !
    CALL mp_sum( mapg_owner, intra_bgrp_comm )
    !
